...
protected:
    ...
    __aicore__ inline void CopyKeyToL1(const RunInfo &info, uint32_t kSize, uint32_t kStart, uint32_t kDealSize,
                                       uint32_t nSize, uint32_t nStart, uint32_t nDealSize, uint32_t nopeDealSize,
                                       uint32_t ropeDealSize, uint64_t l1Offset);
    ...
    __aicore__ inline void DealMm1SingleMKN(const RunInfo &info, uint32_t mSize, uint32_t mStart, uint32_t mDealSize,
                                            uint32_t kSize, uint32_t kStart, uint32_t kDealSize, uint32_t nSize,
                                            uint32_t nStart, uint32_t nDealSize, uint32_t nopeDealSize,
                                            uint32_t ropeDealSize);
    __aicore__ inline void DealMm2SingleMKN(const RunInfo &info, uint32_t mSize, uint32_t mStart, uint32_t mDealSize,
                                            uint32_t kSize, uint32_t kStart, uint32_t kDealSize, uint32_t nSize,
                                            uint32_t nStart, uint32_t nDealSize);
    __aicore__ inline void CopyQGmToL1(uint32_t nopeDealSize, uint32_t ropeDealSize, uint32_t mActSizeAlign,
                                       uint32_t bIdx, uint32_t n2Idx, uint32_t gS1Idx, uint32_t kStart,
                                       uint32_t mActSize);
    __aicore__ inline void CopyQDealSizeToL1(uint32_t DealSize, uint64_t queryL1Offset, uint32_t mActSizeAlign,
    ...
    __gm__ uint8_t *keyRopeAntiquantScale, __gm__ uint8_t *attentionOut, __gm__ uint8_t *softmaxLse)
{
    ...
    InitQBuffer(constInfo.batchSize, constInfo.kvHeadNum, constInfo.gSize, constInfo.qSeqSize, qkTensorD,
                actualSeqLengthsGmQ, constInfo.actualLenQDims, queryGmTensor, queryGm);
    ...
    if (constInfo.ropeSplitMode) {
        ...
        InitKVBuffer(constInfo.batchSize, constInfo.kvSeqSize, actualSeqLengthsGm, constInfo.actualLenDims,
                     constInfo.kvHeadNum, constInfo.kvCacheBlockSize, constInfo.headDimRope, keyRopeGmTensor,
                     keyRopeGm);
    }
                                         ...
                                         FaGmTensor<KV_T, KV_FORMAT> &kvGmTensor, GlobalTensor<KV_T> &kvGm)
{
    ...
    if constexpr (PAGE_ATTENTION) {
        if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_PA_BNBD) {
            kvGmTensor.offsetCalculator.Init(kvHeadNum, kvCacheBlockSize, headDim, blockTableGm,
            ...
            kvGmTensor.offsetCalculator.Init(kvHeadNum, kvCacheBlockSize, d1, d0, blockTableGm,
                                             constInfo.maxBlockNumPerBatch);
        }
    } else {
        if constexpr (GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_BNSD) {
            kvGmTensor.offsetCalculator.Init(batchSize, kvHeadNum, kvSeqSize, headDim, actualSeqLengthsGm,
                                             actualLenDims, constInfo.isKVHasLeftPadding, constInfo.kvLeftPaddingSize);
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::InitMm1GlobalTensor(GlobalTensor<MM_OUT_T> mm1ResGm)
{
    // mm1
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::InitMm2GlobalTensor(GlobalTensor<KV_T> vec1ResGm,
                                                                       GlobalTensor<MM_OUT_T> mm2ResGm)
{
    // mm2
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::UpdateKey(uint32_t bIdx)
{
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::UpdateValue(uint32_t bIdx)
{
    static_assert(GmLayoutParams<KV_FORMAT>::CATEGORY == FormatCategory::GM_KV_BNSD,
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::InitBuffers(TPipe *pipe)
{
    // L1
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::AllocEventID()
{
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::FreeEventID()
{
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::CopyKeyToL1(const RunInfo &info, uint32_t kSize, uint32_t kStart,
                                                               uint32_t kDealSize, uint32_t nSize, uint32_t nStart,
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::CopyValueToL1(const RunInfo &info, uint32_t kSize, uint32_t kStart,
                                                                 uint32_t kDealSize, uint32_t nSize, uint32_t nStart,
                                                                 uint32_t nDealSize, uint64_t l1Offset)
{
    FaL1Tensor<KV_T, L1Format::NZ> dstTensor{.tensor = kvL1Tensor[l1Offset],
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::CopyPrefixAndValueToL1(const RunInfo &info, uint32_t kSize,
                                                                          ...
                                                                          uint32_t nDealSize, uint64_t l1Offset)
{
    ...
    if (normalDealSize > 0) {
        GmKvCoord gmCoord{.bIdx = constInfo.batchContinuous ? info.bIdx : 0U,
                          .n2Idx = info.n2Idx,
                          .s2Idx = (constInfo.systemPrefixLen > s2StartIdx ? constInfo.systemPrefixLen : s2StartIdx) -
                                   constInfo.systemPrefixLen,
                          .dIdx = nStart,
                          .s2DealSize = normalDealSize,
                          .dDealSize = nDealSize};
        dstTensor.tensor = kvL1Tensor[l1Offset + BUFFER_SIZE_BYTE_32B / sizeof(KV_T) * prefixDealSize];
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::CopyQGmToL1(uint32_t nopeDealSize, uint32_t ropeDealSize,
                                                               uint32_t mActSizeAlign, uint32_t bIdx, uint32_t n2Idx,
                                                               uint32_t gS1Idx, uint32_t kStart, uint32_t mActSize)
{
    uint64_t queryL1BaseOffset = (qpBufId % 2) * (L1_QP_SIZE / sizeof(Q_T));
    if (nopeDealSize > 0) {
        CopyQDealSizeToL1(nopeDealSize, queryL1BaseOffset, mActSizeAlign, bIdx, n2Idx, gS1Idx, kStart, mActSize,
                          queryGmTensor);
    }
    ...
    if (ropeDealSize > 0) {
        ...
        CopyQDealSizeToL1(ropeDealSize, queryRopeL1Offset, mActSizeAlign, bIdx, n2Idx, gS1Idx, ropeKStart, mActSize,
                          queryRopeGmTensor);
    }
}

template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::DealMm1SingleMKN(const RunInfo &info, uint32_t mSize,
                                                                    uint32_t mStart, uint32_t mDealSize, uint32_t kSize,
                                                                    uint32_t kStart, uint32_t kDealSize, uint32_t nSize,
                                                                    uint32_t nStart, uint32_t nDealSize,
                                                                    uint32_t nopeDealSize, uint32_t ropeDealSize)
{
    ...
    for (uint32_t m = 0; m < mDealSize; m += M_BASE) {
        ...
        for (uint32_t k = 0; k < kDealSizeAlign; k += K_BASE) {
            ...
            {
                uint64_t kL1Offset =
                    (kvBufId % 2) * (L1_KV_SIZE / sizeof(KV_T)) + k * AttentionCommon::Align(nDealSize, 16U);
                LoadData2DParams loadData2DParams;
...
template <typename FIAT>
__aicore__ inline void FiaBlockCubeNonQuant<FIAT>::DealMm2SingleMKN(const RunInfo &info, uint32_t mSize,
                                                                    uint32_t mStart, uint32_t mDealSize, uint32_t kSize,
                                                                    uint32_t kStart, uint32_t kDealSize, uint32_t nSize,
                                                                    uint32_t nStart, uint32_t nDealSize)
{
        ...
        for (uint32_t k = 0; k < kDealSize; k += K_BASE) {
            ...
            // LoadBToL0
            {
                ...
                for (uint32_t i = 0; i < kLoop; i++) {
                    ...
                    LoadData(bL0Tensor[16 * i * AttentionCommon::Align(nDealSize, 16U)], kvL1Tensor[kL1Offset],
                             loadData2DParams);
                }
            }
            SetFlag<HardEvent::MTE1_M>(L0AB_EVENT0 + l0abBufId % 2);
