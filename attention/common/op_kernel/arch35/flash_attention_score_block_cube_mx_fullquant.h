...
/*!
 * \file flash_attention_score_block_cube_mx_fullquant.h
 * \brief
 */
#ifndef FLASH_ATTENTION_SCORE_BLOCK_CUBE_MX_FULLQUANT_H_
#define FLASH_ATTENTION_SCORE_BLOCK_CUBE_MX_FULLQUANT_H_
...
private:
    ...
    FaGmTensor<QK_SCALE_T, Q_FORMAT> deScaleQGm;
    ...
    // mm2右矩阵V
    typename BlockCubeMxFullquant::KVL1BuffSel<INPUT_T, s2BaseSize, dBaseSize>::Type l1VBuffers;

    BuffersPolicySingleBuffer<BufferType::L1> PScaleBuffers;

    // L0A
    using L0AType = typename BlockCubeMxFullquant::L0ABuffSel<INPUT_T>::Type;
    L0AType mmL0ABuffers;
    // L0B
    using L0BType = typename BlockCubeMxFullquant::L0BBuffSel<INPUT_T, s2BaseSize, dBaseSize>::Type;
    L0BType mmL0BBuffers;
    // L0C
    using L0CType = typename BlockCubeMxFullquant::L0CBuffSel<INPUT_T, s1BaseSize, s2BaseSize, dVBaseSize>::Type;
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void FABlockCubeMxFullquant<TEMPLATE_ARGS>::InitCubeInput(
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void FABlockCubeMxFullquant<TEMPLATE_ARGS>::UnInit()
{
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void FABlockCubeMxFullquant<TEMPLATE_ARGS>::InitLocalBuffer()
{
    constexpr uint32_t mm1QSize = s1BaseSize * dBaseSize * sizeof(INPUT_T);
    constexpr uint32_t mm1QScaleSize = s1BaseSize * dBaseSize / 32 * sizeof(QK_SCALE_T);
    constexpr uint32_t mm1KSize = dBaseSize * s2BaseSize / 2 * sizeof(INPUT_T);
    constexpr uint32_t mm1KScaleSize = s2BaseSize / 2 * dBaseSize / 32 * sizeof(QK_SCALE_T);
    constexpr uint32_t mm2RightSize = (uint32_t)dVTemplateType * s2BaseSize * sizeof(INPUT_T);
    constexpr uint32_t mm2VScaleSize = s2BaseSize * dBaseSize / 32 * sizeof(QK_SCALE_T);

    l1QBuffers.Init((*l1BufferManagerPtr), mm1QSize + mm1QScaleSize);
    l1KBuffers.Init((*l1BufferManagerPtr), mm1KSize + mm1KScaleSize);
                                                   ...
                                                   int32_t subLoop)
{
    ...
    if constexpr (isFp8) {
        if constexpr (isMxfp8FullQuant) {
            IterateBmm1MxFp8(outputBuf, runInfo, constInfo, subLoop);
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    if constexpr (isMxfp8FullQuant) {
        ...
        if constexpr (isPa) {
            ...
            startPos.dIdx = 0;
            PAShape shapeMm2;
            shapeMm2.blockSize = kvCacheBlockSize;
            shapeMm2.headNum = constInfo.n2Size;
            shapeMm2.headDim = constInfo.dSizeV;
            shapeMm2.actHeadDim = constInfo.dSizeV;
            shapeMm2.maxblockNumPerBatch = maxBlockNumPerBatch;
            shapeMm2.copyRowNum = runInfo.s2RealSize;
            if constexpr (isFp8) {
                shapeMm2.copyRowNumAlign = (runInfo.s2RealSize + 31) >> 5 << 5;
            } else {
                shapeMm2.copyRowNumAlign = (runInfo.s2RealSize + 15) >> 4 << 4;
            }
            GlobalTensor<INPUT_T> mm2BGmTensor = GetValueGm(runInfo, constInfo);
            GmCopyInToL1PA<INPUT_T>(mm2BTensor, mm2BGmTensor, blockTableGm, kvLayout, shapeMm2, startPos);
        } else {
            if constexpr (enableKVPrefix) {
                                                                                ...
                                                                                __gm__ uint8_t *deqScaleV)
{
    if constexpr (isFp8) {
        ...
        this->deScaleKGm.gmTensor.SetGlobalBuffer((__gm__ QK_SCALE_T *)deqScaleK);
        this->deScaleVGm.gmTensor.SetGlobalBuffer((__gm__ QK_SCALE_T *)deqScaleV);
    }
}
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if constexpr (bmm2Write2Ub) {
        ...
        fixpipeParamsNz.dstStride = (uint32_t)constInfo.dSizeV; // dstGm 两行之间的间隔
    }
    fixpipeParamsNz.dualDstCtl = 1;
                                                     ...
                                                     RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if constexpr (isPa) {
        ...
        startPos.dIdx = 0;
        PAShape shapeNd;
        shapeNd.blockSize = kvCacheBlockSize;
        shapeNd.headNum = constInfo.n2Size;
        shapeNd.headDim = constInfo.dSize;
        shapeNd.actHeadDim = constInfo.dSize;
        shapeNd.maxblockNumPerBatch = maxBlockNumPerBatch;
        shapeNd.copyRowNum = runInfo.s2RealSize;
        if constexpr (isFp8) {
            shapeNd.copyRowNumAlign = (runInfo.s2RealSize + 31) >> 5 << 5;
        } else {
            shapeNd.copyRowNumAlign = (runInfo.s2RealSize + 15) >> 4 << 4;
        }
        GlobalTensor<INPUT_T> mm1BGmTensor = GetKeyGm(runInfo, constInfo);
        GmCopyInToL1PA<INPUT_T>(mm1BTensor, mm1BGmTensor, blockTableGm, kvLayout, shapeNd, startPos);
    } else {
        if constexpr (enableKVPrefix) {
            if ((runInfo.s2LoopCount + runInfo.s2StartIdx / s2BaseSize) < constInfo.prefixLoopCount) {
    ...
    FixpipeParamsC310<CO2Layout::ROW_MAJOR> fixpipeParamsNd; // L0C→UB
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void FABlockCubeMxFullquant<TEMPLATE_ARGS>::IterateBmm1MxFp8(
    ...
    ConstInfo<isInfer, hasRope> &constInfo, int32_t subLoop)
{
    ...
    uint32_t s2CalcSize;
    if (runInfo.s2RealSize <= s2SplitSize) {
        s2CalcSize = runInfo.s2RealSize;
    } else {
        if (subLoop == 0) {
            s2CalcSize = s2SplitSize;
        } else {
            s2CalcSize = runInfo.s2RealSize - s2SplitSize;
        }
    }
    Buffer<BufferType::L1> mm1A;
    Buffer<BufferType::L1> mm1B;
    uint32_t dstNzC0StrideQNope = (runInfo.s1RealSize + 31) >> 5 << 5;
    uint32_t offsetQScaleByElement = dstNzC0StrideQNope * constInfo.dSize / dTypeScaleRATIO;
    if (unlikely(runInfo.s2LoopCount == 0 && subLoop == 0)) {
        mm1B = l1QBuffers.Get();
    ...
    LocalTensor<INPUT_T> mm1ATensor = mm1A.GetTensor<INPUT_T>();
    uint32_t dstNzC0StrideKNope = (s2CalcSize + 31) >> 5 << 5;
    uint32_t offsetKScaleByElement = dstNzC0StrideKNope * constInfo.dSize / dTypeScaleRATIO;
    LocalTensor<QK_SCALE_T> kMxTensor = mm1A.GetTensor<QK_SCALE_T>(offsetKScaleByElement);
    if constexpr (isPa) {
        Position startPos;
        startPos.bIdx = runInfo.boIdx;
        startPos.n2Idx = runInfo.n2oIdx;
        ...
        nopeShape.copyRowNum = s2CalcSize;
        nopeShape.copyRowNumAlign = (s2CalcSize + 31) >> 5 << 5;
        GlobalTensor<INPUT_T> mm1AGmTensor = GetKeyGm(runInfo, constInfo);
        GmCopyInToL1PA<INPUT_T>(mm1ATensor, mm1AGmTensor, blockTableGm, kvLayout, nopeShape, startPos);
    ...
    FixpipeParamsC310<CO2Layout::ROW_MAJOR> fixpipeParamsMxFp8;
    fixpipeParamsMxFp8.nSize = (runInfo.s1RealSize + 31) >> 5 << 5;
    fixpipeParamsMxFp8.mSize = (s2CalcSize + 7) >> 3 << 3;
    fixpipeParamsMxFp8.srcStride = ((fixpipeParamsMxFp8.mSize + 15) >> 4) << 4;

    if constexpr (useDn && isFp8) {
        fixpipeParamsMxFp8.dstStride = 64;
    } else {
        fixpipeParamsMxFp8.dstStride = fixpipeParamsMxFp8.nSize / 2;
    }

    fixpipeParamsMxFp8.dualDstCtl = 2;
                                                     ...
                                                     RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if constexpr (isPa) {
        ...
        startPos.dIdx = 0;
        PAShape shapeDn;
        shapeDn.blockSize = kvCacheBlockSize;
        shapeDn.headNum = constInfo.n2Size;
        shapeDn.headDim = constInfo.dSize;
        shapeDn.actHeadDim = constInfo.dSize;
        shapeDn.maxblockNumPerBatch = maxBlockNumPerBatch;
        shapeDn.copyRowNum = runInfo.s2RealSize;
        shapeDn.copyRowNumAlign = (runInfo.s2RealSize + 15) >> 4 << 4;
        GlobalTensor<INPUT_T> mm1AGmTensor = GetKeyGm(runInfo, constInfo);
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if (unlikely(runInfo.s2LoopCount == 0)) {
        ...
        InitConstValue(mm2AScaleTensor, {1, s1BaseSize * s2BaseSize / 32 * sizeof(QK_SCALE_T) / 32, 0, 0x7f7f});
    ...
    uint32_t offsetVScaleByElement = dstNzC0StrideKNope * constInfo.dSize;
    LocalTensor<QK_SCALE_T> mm2BScaleTensor = mm2B.GetTensor<QK_SCALE_T>(offsetVScaleByElement);
    mm2B.Wait<HardEvent::MTE1_MTE2>(); // 占用L1B
    int64_t L1Boffset = s2SplitSize * constInfo.dSizeV;
    int64_t gmSplitOffset = s2SplitSize * constInfo.mm2Kb;
    int64_t L1BScaleOffset = vScaleS2SplitSize * constInfo.dSizeV;
    if constexpr (isPa) {
        Position startPos;
        startPos.bIdx = runInfo.boIdx;
        startPos.n2Idx = runInfo.n2oIdx;
        ...
        shapeBmm2MxFp8.blockSize = kvCacheBlockSize;
        shapeBmm2MxFp8.headNum = constInfo.n2Size;
        shapeBmm2MxFp8.headDim = constInfo.dSizeV;
        shapeBmm2MxFp8.actHeadDim = constInfo.dSizeV;
        shapeBmm2MxFp8.maxblockNumPerBatch = maxBlockNumPerBatch;
        shapeBmm2MxFp8.copyRowNum = runInfo.s2RealSize;
        if constexpr (isFp8) {
            shapeBmm2MxFp8.copyRowNumAlign = (runInfo.s2RealSize + 31) >> 5 << 5;
        } else {
            shapeBmm2MxFp8.copyRowNumAlign = (runInfo.s2RealSize + 15) >> 4 << 4;
        }
        PAShape scaleShape = shapeBmm2MxFp8;
        Position scaleStartPos = startPos;
        GlobalTensor<INPUT_T> mm2BGmTensor = GetValueGm(runInfo, constInfo);
        if (runInfo.s2RealSize > s2SplitSize) {
            shapeBmm2MxFp8.copyRowNum = s2SplitSize;
            if constexpr (isFp8) {
                shapeBmm2MxFp8.copyRowNumAlign = (shapeBmm2MxFp8.copyRowNum + 31) >> 5 << 5;
            } else {
                shapeBmm2MxFp8.copyRowNumAlign = (shapeBmm2MxFp8.copyRowNum + 15) >> 4 << 4;
            }
            ...
            LocalTensor<INPUT_T> mm2BTensorTmp = mm2B.GetTensor<INPUT_T>(L1Boffset);
            GmCopyInToL1PA<INPUT_T>(mm2BTensorTmp, mm2BGmTensor, blockTableGm, kvLayout, shapeBmm2MxFp8, startPos);
        } else {
            GmCopyInToL1PA<INPUT_T>(mm2BTensor, mm2BGmTensor, blockTableGm, kvLayout, shapeBmm2MxFp8, startPos);
        }
    ...
    FixpipeParamsC310<CO2Layout::ROW_MAJOR> fixpipeParamsBmm2MxFp8;
    if constexpr (bmm2Write2Ub) {
        fixpipeParamsBmm2MxFp8.nSize = (constInfo.dSizeV + 7) >> 3 << 3;
    } else {
        fixpipeParamsBmm2MxFp8.nSize = constInfo.dSizeV;
    }

    fixpipeParamsBmm2MxFp8.mSize = (runInfo.s1RealSize + 7) >> 3 << 3;
    fixpipeParamsBmm2MxFp8.srcStride = ((runInfo.s1RealSize + 15) / 16) * 16;
    if constexpr (bmm2Write2Ub) {
        fixpipeParamsBmm2MxFp8.dstStride = ((uint32_t)dVTemplateType + 15) >> 4 << 4;
    } else {
        fixpipeParamsBmm2MxFp8.dstStride = (uint32_t)constInfo.dSizeV;
    }
    fixpipeParamsBmm2MxFp8.dualDstCtl = 1;
...
// 判断是否GS1合轴
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline bool FABlockCubeMxFullquant<TEMPLATE_ARGS>::IsGS1Merge(ConstInfo<isInfer, hasRope> &constInfo)
{
...
TEMPLATES_DEF
class FABlockCubeMxFullquantDummy {
public:
    static constexpr bool isFp8 = FABlockCubeMxFullquant<TEMPLATE_ARGS>::isFp8;
    static constexpr bool useDn = FABlockCubeMxFullquant<TEMPLATE_ARGS>::useDn;
...
DEFINE_CUBE_BLOCK_TRAITS(FABlockCubeMxFullquantDummy);
