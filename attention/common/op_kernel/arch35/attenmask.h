        ...
        if constexpr (enableKVPrefix) {
            if ((runInfo.s2LoopCount + runInfo.s2StartIdx / constInfo.s2BaseSize) < constInfo.prefixLoopCount) {
                ...
                s2Offset = runInfo.s2StartIdx +
                           (runInfo.s2LoopCount - constInfo.prefixLoopCount) * constInfo.s2BaseSize +
                                                 ...
                                                 const bool useDn = false, const int32_t subLoop = 0)
{
    if constexpr (hasAtten == true) {
        if (attenMaskInfo.compressMode == static_cast<uint8_t>(AttenMaskCompressMode::NO_COMPRESS_MODE)) {
            return ComputeOffsetForNoCompress<hasAtten, enableKVPrefix, dTemplateType, isInfer, hasRope, isFd>(
        ...
        if (constInfo.layoutType == (uint32_t)LayOutTypeEnum::LAYOUT_TND && !isInfer) {
            ...
            if (attenMaskInfo.compressMode == static_cast<uint8_t>(AttenMaskCompressMode::LEFT_UP_CAUSAL_MODE)) {
                ...
                int64_t transNextTokens =
                    runInfo.actualS2Size - AttentionCommon::Max(runInfo.actualS1Size - tmpNext, 0);
                ...
                if (runInfo.boIdx == attenMaskInfo.bandIndex) {
                    int64_t tmpPre = attenMaskInfo.preTokens;
                    int64_t tmpNext = attenMaskInfo.nextTokens;
                    int64_t transPreTokens =
                        runInfo.actualS1Size - AttentionCommon::Max(runInfo.actualS2Size - tmpPre, 0);
                    int64_t transNextTokens =
                        runInfo.actualS2Size - AttentionCommon::Max(runInfo.actualS1Size - tmpNext, 0);
                    deltaPre = s1Offset - s2Offset - transPreTokens - 1;
        ...
        if (attenMaskInfo.compressMode == static_cast<uint8_t>(AttenMaskCompressMode::LEFT_UP_CAUSAL_MODE)) {
            ...
            if constexpr (hasRope && (dTemplateType == DTemplateType::Aligned576) && isInfer) {
                ...
                deltaCausalOrNext = runInfo.nextTokensOfMlaPerBatch + s1Offset - s2Offset;
                attenMaskInfo.attenMaskOffsetPre = ComputeOffsetForCausal(
                    deltaPre, constInfo.s1BaseSize, constInfo.s2BaseSize, attenMaskInfo.attenMaskS2Size, 0);
            } else {
                if constexpr (isInfer) {
        ...
        GetAttenMaskComputeMode<hasAtten, isInfer, hasRope, isMxfp8FullQuant>(deltaCausalOrNext, deltaPre, s1Offset,
                                                                              runInfo, constInfo, attenMaskInfo);
        int64_t ret = 0;
