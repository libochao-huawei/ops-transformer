                                                  ...
                                                  bool isLast, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if (unlikely(runInfo.s2LoopCount == 0)) { // sOuter循环第一个基本块：搬运0
        ...
        Gm2L1Nd2NzParams.dstNzNStride =
            1; // 转换为NZ矩阵后，ND之间相邻两行在NZ矩阵中起始地址之间的偏移， 单位为Block个数
        ...
        if constexpr (hasRope) {
            ...
            Gm2L1Nd2NzParams.dstNzC0Stride = (Gm2L1Nd2NzParams.nValue + 15) >>
                                             4 << 4; // 转换为NZ矩阵后，相邻Block起始地址之间的偏移， 单位为Block个数
            Gm2L1Nd2NzParams.dstNzNStride =
                1; // 转换为NZ矩阵后，ND之间相邻两行在NZ矩阵中起始地址之间的偏移， 单位为Block个数
            Gm2L1Nd2NzParams.dstNzMatrixStride = 0; // 两个NZ矩阵，起始地址之间的偏移， 单位为元素数量
            DataCopy(mm1ATensor[Gm2L1Nd2NzParams.dstNzC0Stride * constInfo.dSize], this->queryRopeGm[queryRopeOffset],
                     Gm2L1Nd2NzParams);
        }
    ...
    if constexpr (isPa) {
        ...
        GmCopyInToL1HasRopePA<INPUT_T>(mm1BTensor, mm1BRopeTensor, mm1BNopeGmTensor, mm1BRopeGmTensor, blockTableGm,
        ...
        if constexpr (hasRope) {
            ...
            Gm2L1Nd2NzParams.dstNzNStride =
                1; // 转换为NZ矩阵后，ND之间相邻两行在NZ矩阵中起始地址之间的偏移， 单位为Block个数
            Gm2L1Nd2NzParams.dstNzMatrixStride = 0; // 两个NZ矩阵，起始地址之间的偏移， 单位为元素数量
            DataCopy(mm1BTensor[Gm2L1Nd2NzParams.dstNzC0Stride * constInfo.dSize],
                     this->keyRopeGm[keyRopeOffset[runInfo.taskIdMod3]], Gm2L1Nd2NzParams);
        }
    }
    ...
    MMParam param = {
        (uint32_t)runInfo.s1RealSize,                      // singleM 64
                                                         ...
                                                         ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if ((layout == LayOutTypeEnum::LAYOUT_TND || layout == LayOutTypeEnum::LAYOUT_BSH) &&
        runInfo.nextTokensPerBatch < 0) {
                                                  ...
                                                  RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    MatmulN<INPUT_T, INPUT_T, T, 64, 128, 128, ABLayout::MK, ABLayout::KN>(
        mm2AB.GetTensor<INPUT_T>(s2BaseSize * VDsize), mm2AB.GetTensor<INPUT_T>(), mmL0ABuffers, mmL0BBuffers,
