...
public:
    ...
    static constexpr uint32_t l1BaseD =
        isFp8 ? 256 : ((IsSameType<INPUT_T, float>::value) ? (dBaseSize > 128 ? 64 : 128) : 128);
    ...
    if constexpr (isMlaFullQuant) {
        ...
        if constexpr ((dBaseSize > 256) || IsSameType<INPUT_T, float>::value) {
            ...
            */
            ...
            /* fp8场景bmm1右矩阵在L1上全载*/
            constexpr uint32_t mm1RightSize =
                isFp8 ? (dBaseSize * s2BaseSize * sizeof(INPUT_T)) : (s2BaseSize * l1BaseD * sizeof(INPUT_T));
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void
FANoQuantBlockCube<TEMPLATE_ARGS>::IterateBmm1(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &outputBuf,
                                               RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    for (uint32_t n = 0; n < nLoops; ++n) {
        ...
        if constexpr (isPa) {
            ...
            if constexpr (enableKVPrefix) {
                if ((runInfo.s2LoopCount + runInfo.s2StartIdx / s2BaseSize) < constInfo.prefixLoopCount) {
                    ...
                    CopyToL1Nd2Nz<INPUT_T>(mm2BTensor, this->valueSharedPrefixGm.gmTensor[gmOffset + gmNOffset],
                                           runInfo.s2RealSize, realN, constInfo.mm2Kb);
                } else {
                    uint64_t gmOffset = runInfo.keyOffset;
                    CopyToL1Nd2Nz<INPUT_T>(mm2BTensor, GetValueGm(runInfo, constInfo)[gmOffset + gmNOffset],
                                           runInfo.s2RealSize, realN, constInfo.mm2Kb);
                }
            } else {
                uint64_t gmOffset = runInfo.keyOffset;
        ...
        if constexpr (!useDn) {
            fixpipeParams.mSize =
                ((runInfo.s1RealSize + 1) >> 1)
                << 1; // 有效数据不足16行，只需输出部分行即可;L0C上的bmm1结果矩阵M方向的size大小必须是偶数
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    if constexpr (isMlaFullQuant) {
        ...
        if constexpr (IsSameType<INPUT_T, float>::value || (uint32_t)dVTemplateType > 256 ||
            ...
            fixpipeParams.mSize = s1BaseSize; // 有效数据不足16行，只需要输出部分行即可;
                                              // L0C上的bmm1结果矩阵M方向的size大小; 同mmadParams.m
            fixpipeParams.srcStride =
                ((s1BaseSize + 15) / 16) *
                16; // L0C上bmm1结果相邻连续数据片段间隔（前面一个数据块的头与后面数据块的头的间隔）
            if constexpr (!useDn) {
...
FANoQuantBlockCube<TEMPLATE_ARGS>::GetKeyGm(RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    if constexpr (isInfer) {
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline GlobalTensor<INPUT_T>
FANoQuantBlockCube<TEMPLATE_ARGS>::GetValueGm(RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    if constexpr (isInfer) {
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void
FANoQuantBlockCube<TEMPLATE_ARGS>::FixpipeBmm1NdToUb(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &outputBuf,
                                                     Buffer<BufferType::L0C> &mm1ResL0C, RunInfo<isInfer> &runInfo,
                                                     ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if (unlikely(runInfo.s2LoopCount == 0)) { // sOuter循环第一个基本快：搬运0
        ...
        if constexpr (isInfer) {
            if (IsGS1Merge(constInfo)) {
                ...
                if constexpr (IsSameType<INPUT_T, fp8_e5m2_t>::value || IsSameType<INPUT_T, fp8_e4m3fn_t>::value ||
                              IsSameType<INPUT_T, hifloat8_t>::value) {
                    subMSizeAlign = (runInfo.s1RealSize + 31) >>
                                    5 << 5; // NZ矩阵相邻Block起始地址之间的偏移，单位为Block个数，32对齐
                } else {
                    subMSizeAlign = (runInfo.s1RealSize + 15) >>
        ...
        if constexpr (hasRope) {
            uint32_t dstNzC0Stride =
                (runInfo.s1RealSize + 15) >> 4 << 4; // 转换为NZ矩阵后，相邻Block起始地址之间的偏移, 单位为Block个数;
            if constexpr (isFp8) {
                dstNzC0Stride =
                    (runInfo.s1RealSize + 31) >> 5 << 5; // fp8场景在L1上M方向32对齐，防止loadL12L0出现地址越界
            }
            uint64_t gmRopeOffset = this->queryRopeGm.offsetCalculator.GetOffset(
                runInfo.boIdx, runInfo.n2oIdx, runInfo.goIdx, coordInfo[runInfo.taskIdMod3].s1Coord, 0);
            CopyToL1Nd2Nz<INPUT_T>(mm1ATensor[dstNzC0Stride * constInfo.dSize],
                                   this->queryRopeGm.gmTensor[gmRopeOffset], runInfo.s1RealSize, constInfo.dSizeRope,
                                   constInfo.mm1RopeKa);
        }
    ...
    if constexpr (isPa) {
        ...
        if constexpr (hasRope) {
            ...
            GmCopyInToL1HasRopePA<INPUT_T>(mm1BTensor, mm1BRopeTensor, mm1BGmTensor, mm1BRopeGmTensor, blockTableGm,
                                           kvLayout, shape, ropeShape, startPos);
        } else {
        ...
        if constexpr (hasRope) {
            uint32_t dstNzC0Stride =
                (runInfo.s2RealSize + 15) >> 4 << 4; // 转换为NZ矩阵后，相邻Block起始地址之间的偏移, 单位为Block个数;
            if constexpr (isFp8) {
                dstNzC0Stride =
                    (runInfo.s2RealSize + 31) >> 5 << 5; // fp8场景在L1上M方向32对齐，防止loadL12L0出现地址越界
            }
            uint64_t gmRopeOffset = this->keyRopeGm.offsetCalculator.GetOffset(
                runInfo.boIdx, runInfo.n2oIdx, coordInfo[runInfo.taskIdMod3].s2Coord, 0);
    ...
    ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    mm1ResL0C.Wait<HardEvent::FIX_M>(); // 占用
    MMParam param = {
        (uint32_t)runInfo.s2RealSize, (uint32_t)runInfo.s1RealSize,
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void
FANoQuantBlockCube<TEMPLATE_ARGS>::IterateBmm1Nz(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &outputBuf,
                                                 RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    // 计算key的offset
    Buffer<BufferType::L1> mm1A;
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    fixpipeParams.srcStride = ((runInfo.s1RealSize + 15) / 16) *
                              16; // L0C上bmm1结果相邻连续数据片段间隔（前面一个数据块的头与后面数据块的头的间隔）
    if constexpr (bmm2Write2Ub) {
        fixpipeParams.dstStride = ((uint32_t)dVTemplateType + 15) >> 4 << 4;
    } else {
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void
FANoQuantBlockCube<TEMPLATE_ARGS>::IterateBmm1Nd(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &outputBuf,
                                                 RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    // 计算key的offset
    Buffer<BufferType::L1> mm1A;
    ...
    ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if constexpr (isInfer) {
        if constexpr (IsSameType<INPUT_T, fp8_e5m2_t>::value || IsSameType<INPUT_T, fp8_e4m3fn_t>::value ||
                      IsSameType<INPUT_T, hifloat8_t>::value) {
            subMSizeAlign =
                (runInfo.s1RealSize + 31) >> 5 << 5; // NZ矩阵相邻Block起始地址之间的偏移，单位为Block个数，32对齐
        } else {
            subMSizeAlign =
                (runInfo.s1RealSize + 15) >> 4 << 4; // NZ矩阵相邻Block起始地址之间的偏移，单位为Block个数，16对齐
        }
    }
    for (uint32_t k = 0; k < kLoops; k++) {
        ...
        if (unlikely(runInfo.s2LoopCount == 0)) { // sOuter循环第一个基本快：搬运0
            ...
            if constexpr (isInfer) {
                if (IsGS1Merge(constInfo)) { // PFA
                    ...
                    if constexpr (layout == LayOutTypeEnum::LAYOUT_NTD) {
                        CopyToL1Nd2Nz<INPUT_T>(mm1ATensor[k * l1BaseKOffset],
                                               this->queryGm.gmTensor[gmOffset + gmKOffset], runInfo.s1RealSize, realK,
                                               constInfo.mm1Ka);
                    } else {
                        CopyToL1Nd2Nz<INPUT_T>(mm1ATensor[k * l1BaseKOffset],
                                               this->queryGm.gmTensor[runInfo.queryOffset + gmKOffset],
        ...
        MatmulFull<INPUT_T, INPUT_T, T, 128, 128, baseK, ABLayout::MK, ABLayout::KN>(
            mm1A.GetTensor<INPUT_T>()[k * l1BaseKOffset], mm1BTensor, mmL0ABuffers, mmL0BBuffers,
            mm1ResL0C.GetTensor<T>(), param);
...
TEMPLATES_DEF_NO_DEFAULT
__aicore__ inline void
FANoQuantBlockCube<TEMPLATE_ARGS>::IterateBmm1Dn(Buffer<BufferType::UB, SyncType::CROSS_CORE_SYNC_BOTH> &outputBuf,
                                                 RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    Buffer<BufferType::L1> mm1A;
    Buffer<BufferType::L1> mm1B;
    ...
    RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    MatmulN<INPUT_T, INPUT_T, T, 128, 256, 128, ABLayout::MK, ABLayout::KN>(
        mm2A.GetTensor<INPUT_T>(), mm2B.GetTensor<INPUT_T>(), mmL0ABuffers, mmL0BBuffers, mm2ResL0C.GetTensor<T>(),
        param);
