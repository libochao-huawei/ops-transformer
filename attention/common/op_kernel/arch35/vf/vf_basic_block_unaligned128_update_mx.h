...
__simd_vf__ void ProcessVec1UpdateGeneralImpl128Mxfp8FullquantVFSubloop0(
    ...
    const uint32_t dupStride, const uint32_t oriTailN, const uint32_t tailN, const float dScale, uint32_t pltOriTailN,
    uint32_t pltTailN, float divValue, uint32_t pltN, const uint16_t m, const uint32_t pseStride, const float slopes,
    ...
    const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    // 编码（PSE）
    RegTensor<float> vreg_pse_u128u0;
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (pseMode != PseTypeEnum::PSE_NONE_TYPE) {
            if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_TYPE ||
                ...
                if constexpr (IsSameType<pseShiftType, float>::value) {
                    LoadAlign(vreg_pse_u128u0, pseUb + i * pseStride);
                    ...
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_u128u0, vreg_pse_bf16, preg_all_b16);
                    ...
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_u128u0, vreg_pse_f16, preg_all_b16);
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_unroll, vreg_pse_f16_unroll, preg_all_b16);
                }
            }
            Add(vreg_input_s, vreg_input_s, vreg_pse_u128u0, preg_all);
        ...
        if constexpr (hasAtten == 1) {
            ...
            Max(vreg_max_temp, vreg_sel, vreg_sel_unroll_new, preg_all);
            Reduce<Reg::ReduceType::MAX, float, float, Reg::MaskMergeMode::ZEROING>(vreg_max_input, vreg_max_temp,
                                                                                    preg_all);
        } else {
            Select(vreg_input_s_unroll_new, vreg_input_s_unroll, vreg_min, preg_ori_tail_n);
            StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + i * s2BaseSize, vreg_input_s, preg_all);
            StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize + i * s2BaseSize,
                                                         vreg_input_s_unroll_new, preg_tail_n);
            Max(vreg_max_temp, vreg_input_s, vreg_input_s_unroll_new, preg_all);
            Reduce<Reg::ReduceType::MAX, float, float, Reg::MaskMergeMode::ZEROING>(vreg_max_input, vreg_max_temp,
                                                                                    preg_all);
        }
        if constexpr (hasSink) {
        ...
        Sub(vreg_max_input, vreg_max_input, vreg_ln_p_scale, preg_all);
        ...
        StoreUnAlign<float, Reg::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ T *&)tmpMaxUb), vreg_max_input, ureg_max, 1);
    }
    ...
    StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)preLoopMaxUb, vreg_max_in,
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (IsSameType<T2, float>::value) {
            ...
            LoadAlign<T, Reg::LoadDist::DIST_DINTLV_B32>(vreg_input_s, vreg_input_s_unroll, srcUb + i * s2BaseSize);
        }
        ...
        if constexpr (IsSameType<T2, float>::value) {
            ...
            Or<uint16_t, Reg::MaskMergeMode::ZEROING>((Reg::RegTensor<uint16_t> &)vreg_exp_f16,
                                                      (Reg::RegTensor<uint16_t> &)vreg_exp_even_f16,
                                                      (Reg::RegTensor<uint16_t> &)vreg_exp_odd_f16, preg_all_b16);
    ...
    StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)preLoopSumUb, vreg_exp_sum_in, preg_all);
    Mul(vreg_exp_max, vreg_exp_max, vreg_exp_sum_in, preg_all);
    Add(vreg_exp_max, vreg_exp_max, vreg_exp_sum_brc, preg_all);
    StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)expSumUb, vreg_exp_max, preg_all);
    ...
    StoreAlign<fp8_e8m0_t, Reg::StoreDist::DIST_NORM_B8>(((__ubuf__ fp8_e8m0_t *&)pScaleSubLoop0), vreg_pscale_f8e8m0,
                                                         preg_all_b8);
}
...
__simd_vf__ void ProcessVec1UpdateGeneralImpl128Mxfp8FullquantVFSubloop1(
    __ubuf__ T2 *expUb_u128u1, __ubuf__ T2 *x_expUb, __ubuf__ pseShiftType *pseUb, __ubuf__ T *maxUb,
    ...
    __ubuf__ T *tmpMaxUb2, __ubuf__ uint8_t *indexesUb, __ubuf__ uint32_t *maskUb, __ubuf__ uint32_t *maskUbUnroll,
    __ubuf__ uint32_t *dropMaskUb, __ubuf__ fp8_e8m0_t *pScaleSubLoop0, __ubuf__ float *preLoopMaxUb,
    __ubuf__ float *preLoopSumUb, __ubuf__ float *firstLoopSumUb, const uint32_t nPadding, const uint32_t blockStride,
    const uint32_t dupStride, const uint32_t oriTailN, const uint32_t tailN, const float dScale, uint32_t pltOriTailN,
    uint32_t pltTailN, float divValue, uint32_t pltN, const uint16_t m, const uint32_t pseStride, const float slopes,
    ...
    const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    RegTensor<float> vreg_min_u128u1;
    RegTensor<float> vreg_sel;
    RegTensor<float> vreg_sel_unroll;
    RegTensor<float> vreg_sel_unroll_new;
    RegTensor<float> vreg_input_s;
    ...
    // MAX相关
    RegTensor<float> vreg_max_input;
    RegTensor<float> vreg_exp_max_u128u1;
    RegTensor<float> vreg_max_temp;
    RegTensor<float> vreg_sub_loop_update;
    RegTensor<float> vreg_max_pre_loop;
    RegTensor<float> vreg_max_new;
    ...
    RegTensor<float> vreg_exp_odd;
    // POS编码
    RegTensor<float> vreg_pse_u128u1;
    RegTensor<float> vreg_alibi;
    RegTensor<float> vreg_pse_unroll;
    RegTensor<float> vreg_alibi_unroll_u128u1;
    ...
    Duplicate(vreg_min_u128u1, minValue);
    if constexpr (hasSink) {
        Duplicate(vreg_sink_input, sinkValue);
    }
    if constexpr (hasAtten == 1 && isMlaSgd) {
    ...
    if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_TYPE ||
                  pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE) {
        Arange(vreg_alibi, posShift);
        Arange(vreg_alibi_unroll_u128u1, posShift + 64);
    }
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (pseMode != PseTypeEnum::PSE_NONE_TYPE) {
            if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_TYPE ||
                ...
                Abs(vreg_pse_u128u1, vreg_alibi, preg_all);
                Abs(vreg_pse_unroll, vreg_alibi_unroll_u128u1, preg_all);
                if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE) {
                    Sqrt(vreg_pse_u128u1, vreg_pse_u128u1, preg_all);
                    Sqrt(vreg_pse_unroll, vreg_pse_unroll, preg_all);
                }
                Muls(vreg_pse_u128u1, vreg_pse_u128u1, slopes, preg_all);
                Muls(vreg_pse_unroll, vreg_pse_unroll, slopes, preg_all);
                Adds(vreg_alibi, vreg_alibi, -1.0f, preg_all);
                Adds(vreg_alibi_unroll_u128u1, vreg_alibi_unroll_u128u1, -1.0f, preg_all);
            } else { // outer
                if constexpr (IsSameType<pseShiftType, float>::value) {
                    LoadAlign(vreg_pse_u128u1, pseUb + i * pseStride);
                    LoadAlign(vreg_pse_unroll, pseUb + i * pseStride + (s2BaseSize >> 1));
                } else if constexpr (IsSameType<pseShiftType, bfloat16_t>::value) {
                    LoadAlign(vreg_pse_bf16_input, pseUb + i * pseStride);
                    Interleave(vreg_pse_bf16, vreg_pse_bf16_unroll, vreg_pse_bf16_input, vreg_pse_bf16_input);
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_u128u1, vreg_pse_bf16, preg_all_b16);
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_unroll, vreg_pse_bf16_unroll, preg_all_b16);
                } else {
                    LoadAlign(vreg_pse_f16_input, pseUb + i * pseStride);
                    Interleave(vreg_pse_f16, vreg_pse_f16_unroll, vreg_pse_f16_input, vreg_pse_f16_input);
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_u128u1, vreg_pse_f16, preg_all_b16);
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse_unroll, vreg_pse_f16_unroll, preg_all_b16);
                }
            }
            Add(vreg_input_s, vreg_input_s, vreg_pse_u128u1, preg_all);
            Add(vreg_input_s_unroll, vreg_input_s_unroll, vreg_pse_unroll, preg_ori_tail_n);
        }
        if constexpr (pseMode == PseTypeEnum::PSE_OUTER_ADD_MUL_TYPE) {
        ...
        if constexpr (hasAtten == 1) {
            ...
            Select(vreg_sel, vreg_min_u128u1, vreg_input_s, preg_compare);
            Select(vreg_sel_unroll, vreg_min_u128u1, vreg_input_s_unroll, preg_compare_unroll);
            Select(vreg_sel_unroll_new, vreg_sel_unroll, vreg_min_u128u1, preg_ori_tail_n);
            StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + i * s2BaseSize, vreg_sel, preg_all);
            StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize + i * s2BaseSize,
                                                         vreg_sel_unroll_new, preg_tail_n);
            Max(vreg_max_temp, vreg_sel, vreg_sel_unroll_new, preg_all);
            ...
            Select(vreg_input_s_unroll_new, vreg_input_s_unroll, vreg_min_u128u1, preg_ori_tail_n);
            StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + i * s2BaseSize, vreg_input_s, preg_all);
            StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize + i * s2BaseSize,
                                                         vreg_input_s_unroll_new, preg_tail_n);
            Max(vreg_max_temp, vreg_input_s, vreg_input_s_unroll_new, preg_all);
        ...
        Muls(vreg_max_input, vreg_max_input, LN2, preg_all);
        Sub(vreg_max_input, vreg_max_input, vreg_ln_p_scale, preg_all);
        Compare<float, CMPMODE::LE>(preg_compare_max, vreg_max_input, vreg_min_u128u1, preg_all);
        Select(vreg_max_input, vreg_min_u128u1, vreg_max_input, preg_compare_max);
        StoreUnAlign<float, Reg::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ T *&)tmpMaxUb), vreg_max_input, ureg_max, 1);
    }
    StoreUnAlignPost<float, Reg::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ T *&)tmpMaxUb), ureg_max, 0);
    ...
    ExpSub(vreg_exp_max_u128u1, vreg_max_pre_loop, vreg_max_new, preg_all);
    StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)expMaxUb, vreg_exp_max_u128u1, preg_all);
    StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)maxUb, vreg_max_new, preg_all);
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        if constexpr (IsSameType<T2, float>::value) {
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb_u128u1), vreg_exp_even, blockStride, dupStride, preg_all);
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)x_expUb), vreg_exp_odd, blockStride, dupStride, preg_all);
        } else if constexpr (IsSameType<T2, bfloat16_t>::value) {
            Cast<T2, T, castTraitZero>(vreg_exp_even_bf16, vreg_exp_even, preg_all);
            ...
            Or((RegTensor<uint16_t> &)vreg_exp_bf16, (RegTensor<uint16_t> &)vreg_exp_even_bf16,
               (RegTensor<uint16_t> &)vreg_exp_odd_bf16, preg_all_b16);
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb_u128u1), vreg_exp_bf16, blockStride, dupStride, preg_n_b16);
        } else if constexpr (IsSameType<T2, fp8_e5m2_t>::value) {
            ...
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb_u128u1), vreg_exp_merge_f8e5m2, blockStride, dupStride, preg_all_b8_128);
        } else if constexpr (IsSameType<T2, fp8_e4m3fn_t>::value) {
            ...
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb_u128u1), vreg_exp_merge_f8e4m3, blockStride, dupStride, preg_all_b8_128);
        } else if constexpr (IsSameType<T2, int8_t>::value) {
            // 硬件不支持 float → int8 直接转换，需要分两步：float → half → int8
            RegTensor<int8_t> vreg_exp_merge_tmp_int8_u128u1;
            RegTensor<int8_t> vreg_exp_merge_int8;
            MaskReg preg_all_f16 = CreateMask<half, MaskPattern::ALL>();
            uint32_t maskLen = 128;
            MaskReg preg_all_b8_128 = UpdateMask<T2>(maskLen);
            ...
            Or<uint16_t, Reg::MaskMergeMode::ZEROING>((Reg::RegTensor<uint16_t> &)vreg_exp_f16,
                                                      (Reg::RegTensor<uint16_t> &)vreg_exp_even_f16,
                                                      (Reg::RegTensor<uint16_t> &)vreg_exp_odd_f16, preg_all_b16);
            Cast<T2, half, castTrait0>(vreg_exp_merge_tmp_int8_u128u1, vreg_exp_f16, preg_all_b16);
            Reg::Pack<uint8_t, uint16_t, Reg::HighLowPart::LOWEST>(
                (Reg::RegTensor<uint8_t> &)vreg_exp_merge_int8,
                (Reg::RegTensor<uint16_t> &)vreg_exp_merge_tmp_int8_u128u1);
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ int8_t *&)expUb_u128u1), vreg_exp_merge_int8, blockStride, dupStride, preg_all_b8_128);
        } else if constexpr (IsSameType<T2, hifloat8_t>::value) {
            RegTensor<hifloat8_t> vreg_exp_even_hif8;
            RegTensor<hifloat8_t> vreg_exp_odd_hif8;
            RegTensor<hifloat8_t> vreg_exp_merge_tmp_hif8;
            ...
            Gather(vreg_exp_merge_hif8, vreg_exp_merge_tmp_hif8, vreg_exp_merge_hif8_indexes);
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb_u128u1), vreg_exp_merge_hif8, blockStride, dupStride, preg_all_b8_128);
        } else {
            Cast<T2, T, castTraitZero>(vreg_exp_even_f16, vreg_exp_even, preg_all);
            Cast<T2, T, castTraitOne>(vreg_exp_odd_f16, vreg_exp_odd, preg_all);
            Or((RegTensor<uint16_t> &)vreg_exp_f16, (RegTensor<uint16_t> &)vreg_exp_even_f16,
               (RegTensor<uint16_t> &)vreg_exp_odd_f16, preg_all_b16);
            StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb_u128u1), vreg_exp_f16, blockStride, dupStride, preg_n_b16);
        }
    }
    StoreUnAlignPost<float, Reg::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ T *&)tmpExpSumUb), ureg_exp_sum, 0);
    ...
    Mul(vreg_sum_pre, vreg_exp_max_u128u1, vreg_sum_pre, preg_all);
    Add(vreg_sum_pre, vreg_first_sum, vreg_sum_pre, preg_all);
    StoreAlign<T, Reg::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)expSumUb, vreg_sum_pre, preg_all);
    // pScale
    ...
    Mul(vreg_p_scale, vreg_sub_loop_update, vreg_p_scale, preg_all);
    Cast<bfloat16_t, T, castTraitRintZero>(vreg_p_scale_bf16_0, vreg_p_scale, preg_all);
    Cast<fp8_e8m0_t, bfloat16_t, castTraitNoneZero>(vreg_pscale_f8e8m0_0, vreg_p_scale_bf16_0, preg_all_b16);
    Cast<bfloat16_t, T, castTraitRintOne>(vreg_p_scale_bf16_1, vreg_p_scale, preg_all);
    Cast<fp8_e8m0_t, bfloat16_t, castTraitNoneZero>(vreg_pscale_f8e8m0_1, vreg_p_scale_bf16_1, preg_all_b16);
...
__aicore__ inline void ProcessVec1UpdateGeneralImpl128Mxfp8Fullquant(
    const LocalTensor<T2> &dstTensor_u128um, const LocalTensor<uint8_t> &indexesTensor,
