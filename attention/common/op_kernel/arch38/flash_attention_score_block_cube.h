...
struct QL1BuffSel {
    using Type = std::conditional_t<std::is_same_v<INPUT_T, float> ||
                                        (!(std::is_same_v<INPUT_T, fp8_e5m2_t> || std::is_same_v<INPUT_T, fp8_e5m2_t> ||
...
struct KVL1BuffSel {
    using Type = std::conditional_t<(!(std::is_same_v<INPUT_T, fp8_e5m2_t> || std::is_same_v<INPUT_T, fp8_e5m2_t> ||
                                       std::is_same_v<INPUT_T, hifloat8_t>) &&
...
public:
    ...
    static constexpr bool useDn =
        IsDn(true, isInt8, pseMode, hasAtten, hasDrop, s1BaseSize == 128, dTemplateType, hasRope);
    ...
    static constexpr uint32_t l1BaseD =
        isFp8 ? 256 : ((IsSameType<INPUT_T, float>::value) ? (dBaseSize > 128 ? 96 : 128) : 128);
    ...
    for (uint32_t n = 0; n < nLoops; ++n) {
        ...
        if constexpr (isPa) {
            ...
            if (constInfo.dSize != constInfo.dSizeV) {
                gmOffset = this->valueGm.offsetCalculator.GetOffset(
                    coordInfo[runInfo.taskIdMod3].curBIdx, runInfo.n2oIdx, coordInfo[runInfo.taskIdMod3].s2Coord, 0);
            }
            CopyToL1Nd2Nz<INPUT_T>(mm2BTensor, GetValueGm(runInfo, constInfo)[gmOffset + gmNOffset], runInfo.s2RealSize,
                                   realN, constInfo.mm2Kb);
        }
                                        ...
                                        RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    if constexpr (IsSameType<INPUT_T, float>::value || (uint32_t)dVTemplateType > 256 ||
        ...
        MatmulBase<INPUT_T, INPUT_T, L0C_TYPE, 128, (uint32_t)dVTemplateType, 128, ABLayout::MK, ABLayout::KN>(
            mm2A.GetTensor<INPUT_T>(), mm2BTensor, mmL0ABuffers, mmL0BBuffers, mm2ResL0C.GetTensor<L0C_TYPE>(), param);
        mm2B.Set<HardEvent::MTE1_MTE2>(); // 释放L1B
                                          ...
                                          RunInfo<isInfer> &runInfo, ConstInfo<isInfer, hasRope> &constInfo)
{
    ...
    MatmulBase<INPUT_T, INPUT_T, L0C_TYPE, 128, 128, dBaseSize, ABLayout::MK, ABLayout::KN>(
        mm1A.GetTensor<INPUT_T>(), mm1B.GetTensor<INPUT_T>(), mmL0ABuffers, mmL0BBuffers,
        mm1ResL0C.GetTensor<L0C_TYPE>(), param);
    if (unlikely(runInfo.s2LoopCount == runInfo.s2LoopLimit)) {
