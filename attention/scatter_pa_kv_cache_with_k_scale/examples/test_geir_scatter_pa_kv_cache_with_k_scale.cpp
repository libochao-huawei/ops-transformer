 ...
 */
/*!
 * \file test_geir_scatter_pa_kv_cache_with_k_scale.cpp
 * \brief GEIR test demo for ScatterPaKvCacheWithKScale operator
 */
...
#include "nn_other.h"
#include "../op_graph/scatter_pa_kv_cache_with_k_scale_proto.h"
                     ...
                     std::vector<Operator> &outputs, Graph &graph)
{
    ...
    ADD_INPUT(2, value, inDtype, valueShape);
    ADD_INPUT(3, key_cache, inDtype, keyCacheShape);
    ADD_INPUT(4, value_cache, inDtype, valueCacheShape);
    ADD_INPUT(5, slot_mapping, DT_INT32, slotMappingShape);
    ADD_INPUT(6, key_scale, DT_FLOAT, keyScaleShape);
    ADD_INPUT(7, key_scale_cache, DT_FLOAT, keyScaleCacheShape);

    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, key_cache, inDtype, keyCacheShape);
    ADD_OUTPUT(2, value_cache, inDtype, valueCacheShape);
    ADD_OUTPUT(3, key_scale_cache, DT_FLOAT, keyScaleCacheShape);

    // 添加属性（可选属性）
    ADD_INPUT_ATTR(cache_layout, "BNBD");

    outputs.push_back(scatter_op);
    // 添加完毕
    return SUCCESS;
}
