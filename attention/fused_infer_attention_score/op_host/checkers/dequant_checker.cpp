...
ge::graphStatus DequantChecker::CheckDataTypeSupportFullquant(const FiaTilingInfo &fiaInfo)
{
    ...
    if (ge::GRAPH_SUCCESS != CheckValueSupport(inOutDtypeTuple, fullQuantDtypeSupported)) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
            fiaInfo.opName, paramNames.c_str(), incorrectDtypes.c_str(),
...
ge::graphStatus DequantChecker::CheckDequantScaleDtypeGQAPertensor(const FiaTilingInfo &fiaInfo)
{
    ...
    OP_CHECK_IF(ge::GRAPH_SUCCESS != CheckDtypeSupport(quantScale1Desc, QUANT_SCALE1_NAME),
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                    fiaInfo.opName, "quant_scale1", ToString(quantScale1Desc->GetDataType()).c_str(),
    ...
    OP_CHECK_IF(ge::GRAPH_SUCCESS != CheckDtypeSupport(deqScale1Desc, DEQUANT_SCALE1_NAME),
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                    fiaInfo.opName, "dequant_scale1", ToString(deqScale1Desc->GetDataType()).c_str(),
    ...
    OP_CHECK_IF(ge::GRAPH_SUCCESS != CheckDtypeSupport(deqScale2Desc, DEQUANT_SCALE2_NAME),
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                    fiaInfo.opName, "dequant_scale2", ToString(deqScale2Desc->GetDataType()).c_str(),
                    "In per-tensor quant scenario, "
...
ge::graphStatus DequantChecker::CheckDequantScaleDtypeMXFP8Fullquant(const FiaTilingInfo &fiaInfo)
{
    ...
    OP_CHECK_IF(
        fiaInfo.opParamInfo.quantScale1.tensor != nullptr &&
...
ge::graphStatus DequantChecker::CheckDequantModeMLAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(*fiaInfo.opParamInfo.queryQuantMode != PER_TOKEN_HEAD_MODE,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, "query_quant_mode", std::to_string(*fiaInfo.opParamInfo.queryQuantMode).c_str(),
    ...
    OP_CHECK_IF(*fiaInfo.opParamInfo.keyAntiquantMode != PER_CHANNEL_MODE,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, "key_antiquant_mode", std::to_string(*fiaInfo.opParamInfo.keyAntiquantMode).c_str(),
...
ge::graphStatus DequantChecker::CheckDequantModeGQAPertensor(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF((*fiaInfo.opParamInfo.keyAntiquantMode != PER_CHANNEL_MODE ||
                ...
                OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(
                    fiaInfo.opName, "query_quant_mode, key_antiquant_mode and value_antiquant_mode",
...
ge::graphStatus DequantChecker::CheckDequantModeMXFP8Fullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(*fiaInfo.opParamInfo.queryQuantMode != PER_TOKEN_GROUP_MODE,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, "query_quant_mode", std::to_string(*fiaInfo.opParamInfo.queryQuantMode).c_str(),
                                                          ...
                                                          const std::string &inputName) const
{
    OP_CHECK_IF(tensor == nullptr,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, inputName.c_str(), "empty",
                                                             ...
                                                             const std::string &inputName) const
{
    OP_CHECK_IF(tensor != nullptr,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, inputName.c_str(), "not empty",
    ...
    for (const auto &param : quantParams) {
        if (param.tensor != nullptr) {
            OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                ...
                ("In no quant scenario, quant related param: " + std::string(param.paraName) + " must be empty")
                    .c_str());
            return ge::GRAPH_FAILED;
        }
    }
...
ge::graphStatus DequantChecker::CheckExistencePertensorFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    // shapesize != 0
    OP_CHECK_IF((quantScale1Tensor->GetStorageShape().GetShapeSize() == 0 ||
                 deqScale1Tensor->GetStorageShape().GetShapeSize() == 0 ||
                 deqScale2Tensor->GetStorageShape().GetShapeSize() == 0),
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                    fiaInfo.opName, "dequant_scale1, quant_scale1 and dequant_scale2",
...
ge::graphStatus DequantChecker::CheckExistenceMLAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (ge::GRAPH_SUCCESS != CheckTensorNotExistFullquant(fiaInfo, fiaInfo.opParamInfo.quantScale1.tensor,
        ...
        ge::GRAPH_SUCCESS !=
            CheckTensorNotExistFullquant(fiaInfo, fiaInfo.opParamInfo.deqScale1.tensor, quantModeName, "deqScale1") ||
        ge::GRAPH_SUCCESS !=
            CheckTensorNotExistFullquant(fiaInfo, fiaInfo.opParamInfo.deqScale2.tensor, quantModeName, "deqScale2") ||
        ge::GRAPH_SUCCESS != CheckTensorNotExistFullquant(fiaInfo, fiaInfo.opParamInfo.antiquantScale.tensor,
                                                          quantModeName, "antiquantScale") ||
        ge::GRAPH_SUCCESS != CheckTensorNotExistFullquant(fiaInfo, fiaInfo.opParamInfo.keyRopeAntiquantScale.tensor,
                                                          quantModeName, "keyRopeAntiquantScale")) {
...
ge::graphStatus DequantChecker::CheckExistenceFP8GQAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    // fp8 gqa 仅支持PA场景
    if (ge::GRAPH_SUCCESS !=
        CheckTensorExistFullquant(fiaInfo, fiaInfo.opParamInfo.blockTable.tensor, quantModeName, "block_table")) {
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus DequantChecker::CheckFeaturePertensorFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(keyDim == DIM_NUM_5,
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(fiaInfo.opName, "key", "5D",
...
ge::graphStatus DequantChecker::CheckFeatureMLAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
            fiaInfo.opName, "key and value", "In MLA fullquant scenario, key/value storage mode cannot be tensor list"),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(fiaInfo.inputQType == ge::DT_INT8 &&
                ...
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, "input_layout", "PA_BnBsH",
...
ge::graphStatus DequantChecker::CheckFeatureFP8GQAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(
        fiaInfo.kvStorageMode == KvStorageMode::TENSOR_LIST,
        OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
            fiaInfo.opName, "key and value", "In FP8 GQA fullquant scenario, key/value tensorlist is not supported"),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF((fiaInfo.innerPrecise != HIGH_PRECISION) && (fiaInfo.innerPrecise != HIGH_PERFORMANCE),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(fiaInfo.opName, "inner_precise",
                                                      std::to_string(fiaInfo.innerPrecise).c_str(),
                                                      "In FP8 GQA fullquant scenario, inner_precise must be 0 or 1"),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus DequantChecker::CheckDequantScaleQueryMLAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (fiaInfo.qLayout == FiaLayout::BSH) {
        if ((queryDimNum != dequantScaleQueryDimNum)) {
            ...
            OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(fiaInfo.opName, "dequant_scale_query and query", dimStr.c_str(),
                                                      reasonMsg.c_str());
            return ge::GRAPH_FAILED;
        }
        ...
        if ((queryInputShape.GetDim(DIM_NUM_0) != dequantScaleQueryShape.GetDim(DIM_NUM_0)) ||
            ...
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "dequant_scale_query", shapeStr.c_str(),
                                                  reasonMsg.c_str());
            return ge::GRAPH_FAILED;
        }
        ...
        if ((dequantScaleQueryDimNum != queryDimNum - NUM1)) {
            ...
            OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(fiaInfo.opName, "dequant_scale_query", dimStr.c_str(),
                                                     reasonMsg.c_str());
            return ge::GRAPH_FAILED;
        }
        ...
        for (uint32_t i = 0; i < dequantScaleQueryDimNum; i++) {
            if ((queryInputShape.GetDim(i) != dequantScaleQueryShape.GetDim(i))) {
                ...
                std::string reasonMsg = "In MLA fullquant scenario, the " + std::to_string(i) +
                                        "th dim of dequantScaleQuery should be equal to the " + std::to_string(i) +
                                        "th dim of query";
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "dequant_scale_query", shapeStr.c_str(),
                                                      reasonMsg.c_str());
                return ge::GRAPH_FAILED;
...
ge::graphStatus DequantChecker::CheckDequantScaleBnNBsDShapeMXFP8(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(((ge::GRAPH_SUCCESS != CheckTensorContiguous(keyAntiquantScaleDimNum, keyAntiquantScaleShape,
                                                             fiaInfo.kScaleStrides, dimIndex)) &&
                 (dimIndex != 0 && dimIndex != 1)),
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(fiaInfo.opName, "keyAntiquantScale",
                                                         "In MXFP8 Fullquant BnNBsD scenario, only 0th and 1st axis of "
                                                         "keyAntiquantScale can be non-contiguous, the " +
                                                             std::to_string(dimIndex) +
                                                             "th axis of keyAntiquantScale must be contiguous"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(((ge::GRAPH_SUCCESS != CheckTensorContiguous(valueAntiquantScaleDimNum, valueAntiquantScaleShape,
                                                             fiaInfo.vScaleStrides, dimIndex)) &&
                 (dimIndex != 0 && dimIndex != 1)),
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                    fiaInfo.opName, "valueAntiquantScale",
    ...
    if ((fiaInfo.totalBlockNum != keyAntiquantScaleShape.GetDim(DIM_NUM_0)) ||
        ...
        std::string reasonMsg =
            "In MXFP8 fullquant scenario, when layout of key is " +
    ...
    if ((fiaInfo.totalBlockNum != valueAntiquantScaleShape.GetDim(DIM_NUM_0)) ||
        ...
        std::string reasonMsg =
            "In MXFP8 fullquant scenario, when layout of value is " +
            std::string(LayoutToSerialString(fiaInfo.kvLayout)) +
...
ge::graphStatus DequantChecker::CheckDequantScaleNZShapeMXFP8(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(
        ((ge::GRAPH_SUCCESS !=
         ...
         (dimIndex != 0 && dimIndex != 1)),
        OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
            fiaInfo.opName, "keyAntiquantScale",
        ...
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        ((ge::GRAPH_SUCCESS != CheckTensorContiguous(valueAntiquantScaleDimNum, valueAntiquantScaleShape,
                                                     fiaInfo.vScaleStrides, dimIndex)) &&
         (dimIndex != 0 && dimIndex != 1)),
        OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
            fiaInfo.opName, "valueAntiquantScale",
    ...
    if ((fiaInfo.totalBlockNum != keyAntiquantScaleShape.GetDim(DIM_NUM_0)) ||
        ...
        (CeilDivision(fiaInfo.qkHeadDim, mxfp8BlockSize) != keyAntiquantScaleShape.GetDim(DIM_NUM_3)) ||
        (d0 != keyAntiquantScaleShape.GetDim(DIM_NUM_4)) || (keyAntiquantScaleShape.GetDim(DIM_NUM_5) != 2)) {
        std::string shapeStr = ToStringRaw(keyAntiquantScaleShape);
        std::string reasonMsg =
            "In MXFP8 fullquant scenario when layout of key is " + std::string(LayoutToSerialString(fiaInfo.kvLayout)) +
    ...
    if ((fiaInfo.totalBlockNum != valueAntiquantScaleShape.GetDim(DIM_NUM_0)) ||
        ...
        (d0 != valueAntiquantScaleShape.GetDim(DIM_NUM_4)) || (valueAntiquantScaleShape.GetDim(DIM_NUM_5) != 2)) {
        std::string shapeStr = ToStringRaw(valueAntiquantScaleShape);
        std::string reasonMsg =
            "In MXFP8 fullquant scenario, when layout of value is " +
            std::string(LayoutToSerialString(fiaInfo.kvLayout)) +
...
ge::graphStatus DequantChecker::CheckDequantScaleShapeMXFP8(const FiaTilingInfo &fiaInfo) const
{
    ...
    if ((dequantScaleQueryDimNum != 4 && dequantScaleQueryDimNum != 5)) {
        ...
        std::string reasonMsg = "In MXFP8 fullquant scenario, when the layout of query is " +
                                ...
                                " and the layout of key/value is " +
                                std::string(LayoutToSerialString(fiaInfo.kvLayout)) +
    ...
    if ((keyAntiquantScaleDimNum != scaleKVDim || valueAntiquantScaleDimNum != scaleKVDim)) {
        std::string dimStr =
            std::to_string(keyAntiquantScaleDimNum) + "D and " + std::to_string(valueAntiquantScaleDimNum) + "D";
        std::string reasonMsg = "In MXFP8 fullquant scenario, when the layout of query is " +
                                std::string(LayoutToSerialString(fiaInfo.qLayout)) +
                                " and the layout of key/value is " +
                                std::string(LayoutToSerialString(fiaInfo.kvLayout)) +
                                ", the shape dim of keyAntiquantScale and "
                                "valueAntiquantScale should both be " +
                                std::to_string(scaleKVDim) + "D";
    ...
    if (!enableMxfp8Decode && dequantScaleQueryDimNum == DIM_NUM_5) {
        OP_LOGW(fiaInfo.opName,
                ...
                "which corresponds to [%ld, %ld, %ld, %ld] in this case.",
                queryInputShape.GetDim(DIM_NUM_0), fiaInfo.n1Size,
                CeilDivision(queryInputShape.GetDim(DIM_NUM_2), mxfp8BlockSize), 2);
    }
    if (enableMxfp8Decode && dequantScaleQueryDimNum == DIM_NUM_4) {
        OP_LOGW(fiaInfo.opName,
                "In the mxfp8 decode scenario, to achieve better performance, "
                "the query scale is recommended to use the shape [N2, T, G, D//64, 2], "
                "which corresponds to [%ld, %ld, %ld, %ld, %ld] in this case.",
...
ge::graphStatus DequantChecker::CheckQuantScale1ShapeFP8GQA(const FiaTilingInfo &fiaInfo) const
{
    ...
    if ((quantScale1Tensor != nullptr) &&
        (quantScale1Tensor->GetStorageShape().GetDimNum() != NUM1 || quantScale1Tensor->GetShapeSize() != NUM1)) {
        std::string shapeStr = ToStringRaw(quantScale1Tensor->GetStorageShape());
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "quant_scale1", shapeStr.c_str(),
...
ge::graphStatus DequantChecker::CheckDequantScaleShapeFP8GQA(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(dequantScaleQueryDimNum != 2,
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                    fiaInfo.opName, "dequant_scale_query", std::to_string(dequantScaleQueryDimNum).c_str(),
    ...
    if (fiaInfo.n1Size != dequantScaleQueryShape.GetDim(DIM_NUM_0) ||
        queryInputShape.GetDim(DIM_NUM_1) != dequantScaleQueryShape.GetDim(DIM_NUM_1)) {
        std::string actualShape = "[" + std::to_string(dequantScaleQueryShape.GetDim(DIM_NUM_0)) + ", " +
                                  std::to_string(dequantScaleQueryShape.GetDim(DIM_NUM_1)) + "]";
        std::string expectedShape =
            "[" + std::to_string(fiaInfo.n1Size) + ", " + std::to_string(queryInputShape.GetDim(DIM_NUM_1)) + "]";
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "dequant_scale_query", actualShape.c_str(),
                                              ("In FP8 GQA fullquant scenario, "
                                               "the shape of dequant_scale_query should be " +
                                               expectedShape + " (NT layout).")
                                                  .c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    OP_CHECK_IF(keyAntiquantScaleDimNum != DIM_NUM_3,
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                    fiaInfo.opName, "key_antiquant_scale", std::to_string(keyAntiquantScaleDimNum).c_str(),
    ...
    if (fiaInfo.totalBlockNum != keyAntiquantScaleShape.GetDim(DIM_NUM_0) ||
        fiaInfo.n2Size != keyAntiquantScaleShape.GetDim(DIM_NUM_1) ||
        fiaInfo.blockSize != keyAntiquantScaleShape.GetDim(DIM_NUM_2)) {
        std::string actualShape = "[" + std::to_string(keyAntiquantScaleShape.GetDim(DIM_NUM_0)) + ", " +
                                  std::to_string(keyAntiquantScaleShape.GetDim(DIM_NUM_1)) + ", " +
                                  std::to_string(keyAntiquantScaleShape.GetDim(DIM_NUM_2)) + "]";
        std::string expectedShape = "[" + std::to_string(fiaInfo.totalBlockNum) + ", " +
                                    std::to_string(fiaInfo.n2Size) + ", " + std::to_string(fiaInfo.blockSize) + "]";
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "key_antiquant_scale", actualShape.c_str(),
                                              ("In FP8 GQA fullquant scenario, "
                                               "the shape of key_antiquant_scale should be " +
                                               expectedShape + " (BnNBs layout).")
                                                  .c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    OP_CHECK_IF(((ge::GRAPH_SUCCESS != CheckTensorContiguous(keyAntiquantScaleDimNum, keyAntiquantScaleShape,
                ...
                OP_LOGE(fiaInfo.opName,
                        ...
                        "but currently the non-contiguous dimension is dimension %d",
                        dimIndex),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(valueAntiquantScaleDimNum != 1,
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                    fiaInfo.opName, "value_antiquant_scale", std::to_string(valueAntiquantScaleDimNum).c_str(),
...
ge::graphStatus DequantChecker::CheckStrideFP8GQAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    // key_antiquant_scale BnNBs: 只校验BS轴 stride[2]==1
    OP_CHECK_IF(fiaInfo.kScaleStrides == nullptr,
                OP_LOGE(fiaInfo.opName, "In FP8 GQA fullquant scenario, dequant_scale_key stride is nullptr"),
                return ge::GRAPH_FAILED);
    if (fiaInfo.kScaleStrides->GetDimNum() >= 2) {
        if (fiaInfo.kScaleStrides->GetStride(2) != 1) {
            std::string expStr = "(..., ..., 1)";
            std::string actStr = "(..., ..., " + std::to_string(fiaInfo.kScaleStrides->GetStride(2)) + ")";
            OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                fiaInfo.opName, "dequant_scale_key",
                ("dequant_scale_key stride is " + actStr + ", expected " + expStr).c_str());
            return ge::GRAPH_FAILED;
        }
    }

    // key BnNBsD: 只校验BS轴 stride[2]==qkHeadDim 和 D轴 stride[3]==1
    OP_CHECK_IF(fiaInfo.keyStrides == nullptr,
                OP_LOGE(fiaInfo.opName, "In FP8 GQA fullquant scenario, key stride is nullptr"),
                return ge::GRAPH_FAILED);
    if (fiaInfo.keyStrides->GetDimNum() >= 4) {
        const uint64_t expectedBs = static_cast<uint64_t>(fiaInfo.qkHeadDim);
        if (fiaInfo.keyStrides->GetStride(3) != 1 || fiaInfo.keyStrides->GetStride(2) != expectedBs) {
            std::string expStr = "(..., ..., " + std::to_string(expectedBs) + ", 1)";
            std::string actStr = "(..., ..., " + std::to_string(fiaInfo.keyStrides->GetStride(2)) + ", " +
                                 std::to_string(fiaInfo.keyStrides->GetStride(3)) + ")";
            OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(fiaInfo.opName, "key",
                                                     ("key stride is " + actStr + ", expected " + expStr).c_str());
            return ge::GRAPH_FAILED;
        }
    }

    // value BnNBsD: 只校验BS轴 stride[2]==vHeadDim 和 D轴 stride[3]==1
    OP_CHECK_IF(fiaInfo.valueStrides == nullptr,
                OP_LOGE(fiaInfo.opName, "In FP8 GQA fullquant scenario, value stride is nullptr"),
                return ge::GRAPH_FAILED);
    if (fiaInfo.valueStrides->GetDimNum() >= 4) {
        const uint64_t expectedBs = static_cast<uint64_t>(fiaInfo.vHeadDim);
        if (fiaInfo.valueStrides->GetStride(3) != 1 || fiaInfo.valueStrides->GetStride(2) != expectedBs) {
            std::string expStr = "(..., ..., " + std::to_string(expectedBs) + ", 1)";
            std::string actStr = "(..., ..., " + std::to_string(fiaInfo.valueStrides->GetStride(2)) + ", " +
                                 std::to_string(fiaInfo.valueStrides->GetStride(3)) + ")";
            OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(fiaInfo.opName, "value",
                                                     ("value stride is " + actStr + ", expected " + expStr).c_str());
            return ge::GRAPH_FAILED;
        }
    }
...
ge::graphStatus DequantChecker::CheckInputDTypeFullquant(const FiaTilingInfo &fiaInfo)
{
    if (enableQPerTokenHeadKVPerTensor_) { // MLA 全量化 QKV : fp8_e4m3/int8
        if (!(fiaInfo.inputQType == ge::DT_FLOAT8_E4M3FN || fiaInfo.inputQType == ge::DT_INT8 ||
            ...
            std::string incorrectDtypes = ToString(fiaInfo.inputQType) + ", " + ToString(fiaInfo.inputKvType);
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                fiaInfo.opName, paramNames.c_str(), incorrectDtypes.c_str(),
        ...
        OP_CHECK_IF(fiaInfo.outputType != ge::DT_BF16,
                    OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                        fiaInfo.opName, "attention_out", ToString(fiaInfo.outputType).c_str(),
        ...
        if (!(fiaInfo.inputQRopeType == ge::DT_BF16 && fiaInfo.inputKRopeType == ge::DT_BF16)) {
            ...
            std::string incorrectDtypes = ToString(fiaInfo.inputQRopeType) + ", " + ToString(fiaInfo.inputKRopeType);
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                fiaInfo.opName, paramNames.c_str(), incorrectDtypes.c_str(),
        ...
        if (fiaInfo.ropeMode == RopeMode::ROPE_SPLIT) {
            if (!(fiaInfo.inputQRopeType == ge::DT_BF16 && fiaInfo.inputKRopeType == ge::DT_BF16)) {
                std::string paramNames = "query_rope and key_rope";
                std::string incorrectDtypes =
                    ToString(fiaInfo.inputQRopeType) + ", " + ToString(fiaInfo.inputKRopeType);
        ...
        OP_CHECK_IF(!(fiaInfo.inputQType == ge::DT_FLOAT8_E4M3FN),
                    OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                        fiaInfo.opName, "query, key/value", qkvDtypes.c_str(),
                        "In FP8 GQA fullquant scenario, query and key/value dtype should be FLOAT8_E4M3FN"),
                    return ge::GRAPH_FAILED);

        OP_CHECK_IF(fiaInfo.outputType != ge::DT_BF16 && fiaInfo.outputType != ge::DT_FLOAT16,
                    OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                        fiaInfo.opName, "attention_out", DataTypeToSerialString(fiaInfo.outputType).c_str(),
...
ge::graphStatus DequantChecker::CheckInputLayoutMLAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    const std::string inputLayout = fiaInfo.opParamInfo.layOut;
    const std::vector<std::string> supportedLayoutListFP8 = {"BSH",      "BSND",      "BNSD",      "TND",
    ...
    if ((fiaInfo.inputQType == ge::DT_FLOAT8_E4M3FN || fiaInfo.inputQType == ge::DT_HIFLOAT8) &&
        ...
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            fiaInfo.opName, "input_layout", fiaInfo.opParamInfo.layOut,
    ...
    if (fiaInfo.inputQType == ge::DT_INT8 && (std::find(supportedLayoutListINT8.begin(), supportedLayoutListINT8.end(),
                                                        inputLayout) == supportedLayoutListINT8.end())) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(fiaInfo.opName, "input_layout", fiaInfo.opParamInfo.layOut,
...
ge::graphStatus DequantChecker::CheckInputLayoutFP8GQAFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(fiaInfo.opName, "input_layout", LayoutToSerialString(fiaInfo.kvLayout),
...
ge::graphStatus DequantChecker::CheckN1SizeFullquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF((fiaInfo.n1Size % fiaInfo.n2Size != 0),
                OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(
                    fiaInfo.opName, "num_heads and num_key_value_heads",
...
ge::graphStatus DequantChecker::CheckN2SizeFullquant(const FiaTilingInfo &fiaInfo) const
{
    if (enableQPerTokenHeadKVPerTensor_) {
        ...
        OP_CHECK_IF((fiaInfo.n2Size > N2_LIMIT || fiaInfo.n2Size < NUM1),
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                        fiaInfo.opName, "num_key_value_heads", std::to_string(fiaInfo.n2Size).c_str(),
    ...
    if (enableQPerTokenHeadKVPerTensor_) { // G <= 128
        if (fiaInfo.gSize < NUM1 || fiaInfo.gSize > NUM_128) {
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                fiaInfo.opName, "query and key", shapesStr.c_str(),
...
ge::graphStatus DequantChecker::CheckDSizeFullquant(const FiaTilingInfo &fiaInfo) const
{
    if (enableQPerTokenHeadKVPerTensor_) {
        ...
        if (!((fiaInfo.qkHeadDim == NUM_128 && fiaInfo.vHeadDim == NUM_128) ||
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                fiaInfo.opName, "query and value", shapeMsg.c_str(),
                "In the MXFP8 full quant scenario, the axis D of query and value must both be 64 or both be 128");
            return ge::GRAPH_FAILED;
        }
    } else if (enableQKPerTokenHeadVPerHead_) {
        std::string dimValues = "query and key headDim=" + std::to_string(fiaInfo.qkHeadDim) +
                                ", value headDim=" + std::to_string(fiaInfo.vHeadDim);
        ...
        if (fiaInfo.qkHeadDim > D_LIMIT || fiaInfo.qkHeadDim < NUM1) {
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                fiaInfo.opName, "query and key", shapeMsg.c_str(),
        ...
        if (fiaInfo.qkHeadDim != fiaInfo.vHeadDim) {
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(fiaInfo.opName, "value and key", shapeMsg.c_str(),
                                                   reasonMsg.c_str());
            return ge::GRAPH_FAILED;
        }
    }
    ...
    const FiaTilingInfo &fiaInfo, ge::DataType inputKvType, const gert::Tensor *keyAntiquantScaleTensor) const
{
    ...
    if (inputKvType == ge::DT_INT4 || inputKvType == ge::DT_INT32) {
        OP_CHECK_IF(
            (keyAntiquantScaleShape == expectedShape1),
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                                                   "The dtype of key and value must be INT8 "
                                                   "when keyAntiquantMode and valueAntiquantMode are per-tensor mode"),
            return ge::GRAPH_FAILED);
    } else {
        OP_CHECK_IF(
            (keyAntiquantScaleShape == expectedShape1) && inputKvType != ge::DT_INT8,
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                                                   "The dtype of key and value must be INT8 "
                                                   "when keyAntiquantMode and valueAntiquantMode are per-tensor mode"),
            return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF((inputKvType != ge::DT_INT8 && inputKvType != ge::DT_INT4 && inputKvType != ge::DT_HIFLOAT8 &&
                ...
                OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                    fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                    "The dtype of key and value must be INT8, INT4(INT32), "
                                                                  ...
                                                                  ge::DataType inputKvType) const
{
    ...
    OP_CHECK_IF((inputKvType != ge::DT_INT8 && inputKvType != ge::DT_INT4),
                OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                    fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                    "The dtype of key and value must be INT8 or INT4 "
    ...
    OP_CHECK_IF((fiaInfo.inputKvType == ge::DT_INT8 &&
                ...
                OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                    fiaInfo.opName, "query and attention_out", ToString(inputKvType).c_str(),
                    "The dtype of query and attention_out must be FLOAT16 "
                    "when keyAntiquantMode is per-channel mode and valueAntiquantMode is per-token mode "
                                                                     ...
                                                                     ge::DataType inputKvType) const
{
    ...
    OP_CHECK_IF((inputKvType != ge::DT_INT8 && inputKvType != ge::DT_INT4 && inputKvType != ge::DT_FLOAT8_E4M3FN),
                OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                    fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                    "The dtype of key and value must be INT8, INT4 or "
                                                                       ...
                                                                       ge::DataType inputKvType) const
{
    ...
    OP_CHECK_IF(
        (inputKvType != ge::DT_INT8 && inputKvType != ge::DT_FLOAT8_E4M3FN),
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                                               "The dtype of key and value must be INT8 or FLOAT8_E4M3FN "
                                                                         ...
                                                                         ge::DataType inputKvType) const
{
    ...
    OP_CHECK_IF(
        (inputKvType != ge::DT_INT8 && inputKvType != ge::DT_INT4),
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                                               "The dtype of key and value must be INT8 or INT4 "
                                                                          ...
                                                                          ge::DataType inputKvType) const
{
    ...
    OP_CHECK_IF(
        (inputKvType != ge::DT_FLOAT4_E2M1),
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(fiaInfo.opName, "key and value", ToString(inputKvType).c_str(),
                                               "The dtype of key and value must be FLOAT4_E2M1 "
...
ge::graphStatus DequantChecker::CheckScaleExistenceForAntiquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF(keyAntiquantScaleTensor == nullptr || valueAntiquantScaleTensor == nullptr,
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                    fiaInfo.opName, "key_antiquant_scale and value_antiquant_scale",
                    "In antiquant scenario, keyAntiquantScale and valueAntiquantScale must exist"),
                return ge::GRAPH_FAILED);
    // 不支持KV不分离场景
    OP_CHECK_IF((antiquantMode != 0),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, "antiquant_mode", std::to_string(antiquantMode).c_str(),
    ...
    OP_CHECK_IF((fiaInfo.opParamInfo.antiquantScale.tensor != nullptr),
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                    fiaInfo.opName, "antiquant_scale",
    ...
    OP_CHECK_IF((fiaInfo.opParamInfo.antiquantOffset.tensor != nullptr),
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
                    fiaInfo.opName, "antiquant_offset",
...
ge::graphStatus DequantChecker::CheckDescExistenceForAntiquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    OP_CHECK_IF((fiaInfo.opParamInfo.valueAntiquantOffset.tensor != nullptr &&
...
ge::graphStatus DequantChecker::CheckFeatureForAntiquant(const FiaTilingInfo &fiaInfo)
{
    if (ge::GRAPH_SUCCESS != CheckFeatureLayoutForAntiquant(fiaInfo) ||
        ...
        ge::GRAPH_SUCCESS != CheckStrideForAntiquant(fiaInfo)) {
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus DequantChecker::CheckFeatureLayoutForAntiquant(const FiaTilingInfo &fiaInfo) const
{
    ...
    const std::string inputLayout = fiaInfo.opParamInfo.layOut;
    const std::vector<std::string> supportedLayoutList = {"BSH", "BSND", "BNSD", "BNSD_BSND", "TND"};

    if ((std::find(supportedLayoutList.begin(), supportedLayoutList.end(), inputLayout) == supportedLayoutList.end())) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            fiaInfo.opName, "input_layout", inputLayout.c_str(),
            "In antiquant scenario, inputLayout only supports BSH, BNSD, BSND, BNSD_BSND or TND");
        return ge::GRAPH_FAILED;
    }

    OP_CHECK_IF((fiaInfo.inputKvType == ge::DT_INT8 && inputLayout == "TND"),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    fiaInfo.opName, "input_layout", inputLayout.c_str(),
    ...
    if (fiaInfo.s1Size == 1) {
        OP_CHECK_IF(inputLayout == "BNSD_BSND",
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                        fiaInfo.opName, "input_layout", "BNSD_BSND",
...
ge::graphStatus DequantChecker::CheckFeatureQuerySForAntiquant(const FiaTilingInfo &fiaInfo) const
{
    if (fiaInfo.s1Size > 1) {
        ...
        if ((keyAntiquantMode == PER_CHANNEL_MODE || keyAntiquantMode == PER_TOKEN_MODE) &&
            ...
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                fiaInfo.opName, "query and attention_out", dtypeMsg.c_str(),
        ...
        if ((keyAntiquantMode == PER_CHANNEL_MODE || keyAntiquantMode == PER_TOKEN_MODE) &&
            ...
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "query", std::to_string(fiaInfo.s1Size).c_str(),
                                                  "When the dtype of key/value is INT8 and keyAntiquantMode is 0 or 1, "
        ...
        if (fiaInfo.inputKvType == ge::DT_INT4 || fiaInfo.inputKvType == ge::DT_INT32) {
            ...
            OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                fiaInfo.opName, "key/value", dtypeMsg.c_str(),
...
ge::graphStatus DequantChecker::CheckScaleTypeForAntiquant(const FiaTilingInfo &fiaInfo)
{
    ...
    if (keyAntiquantMode == PER_CHANNEL_MODE && valueAntiquantMode == PER_CHANNEL_MODE) {
        ...
        OP_CHECK_IF(
            (keyAntiquantScaleDesc->GetDataType() != queryDesc->GetDataType()),
            OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                fiaInfo.opName, "key_antiquant_scale", ToString(keyAntiquantScaleDesc->GetDataType()).c_str(),
                "The dtype of "
            ...
            return ge::GRAPH_FAILED);
        OP_CHECK_IF(
            (valueAntiquantScaleDesc->GetDataType() != queryDesc->GetDataType()),
            OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
...
ge::graphStatus DequantChecker::CheckOffsetTypeForAntiquant(const FiaTilingInfo &fiaInfo)
{
    ...
    if (keyAntiquantOffsetDesc != nullptr && keyAntiquantScaleDesc != nullptr) {
        OP_CHECK_IF((keyAntiquantOffsetDesc->GetDataType() != keyAntiquantScaleDesc->GetDataType()),
                    OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                        fiaInfo.opName, "key_antiquant_offset", ToString(keyAntiquantOffsetDesc->GetDataType()).c_str(),
                        "The dtype of "
    ...
    if (valueAntiquantOffsetDesc != nullptr && valueAntiquantScaleDesc != nullptr) {
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(fiaInfo.opName, "value_antiquant_offset",
                                                  ToString(valueAntiquantOffsetDesc->GetDataType()).c_str(),
                                                  "The dtype of "
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerChannelPerTensorMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_1) {
        ...
        if ((keyAntiquantScaleTensorShape != expectedShapeH) && (keyAntiquantScaleTensorShape != expectedShape1)) {
            ...
            std::string reasonMsg = "The shape of keyAntiquantScale must be [H(" +
                                    std::to_string(numKeyValueHeads * headDim) +
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_2) {
        ...
        if ((keyAntiquantScaleTensorShape != expectedShape1H) && (keyAntiquantScaleTensorShape != expectedShapeND)) {
            ...
            std::string reasonMsg = "The shape of keyAntiquantScale must be [1, H(" +
                                    std::to_string(numKeyValueHeads * headDim) + ")] or [N(" +
                                    std::to_string(numKeyValueHeads) + "), D(" + std::to_string(headDim) +
                                    ")] when keyAntiquantMode is per-channel mode and "
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_3) {
        ...
        if ((keyAntiquantScaleTensorShape != expectedShape1ND) && (keyAntiquantScaleTensorShape != expectedShapeN1D)) {
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg =
                "The shape of keyAntiquantScale must be [1, N(" + std::to_string(numKeyValueHeads) + "), D(" +
                std::to_string(headDim) + ")] or [N(" + std::to_string(numKeyValueHeads) + "), 1, D(" +
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_4) {
        ...
        if ((keyAntiquantScaleTensorShape != expectedShape1N1D) &&
            ...
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg =
                "The shape of keyAntiquantScale must be [1, N(" + std::to_string(numKeyValueHeads) + "), 1, D(" +
                std::to_string(headDim) + ")] or [1, 1, N(" + std::to_string(numKeyValueHeads) + "), D(" +
                std::to_string(headDim) +
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerTokenMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_2) {
        ...
        if (keyAntiquantScaleTensorShape.GetDim(DIM_NUM_0) != batchSize ||
            ...
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg = "The shape of key_antiquant_scale must be [1, B(" + std::to_string(batchSize) +
                                    "), >=S(" + std::to_string(seqLength) +
                                    ")] or "
                                    "[B(" +
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerTensorHeadMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_1) {
        ...
        if (keyAntiquantScaleTensorShape != expectedShapeN) {
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg = "The shape of keyAntiquantScale must be [N(" + std::to_string(numKeyValueHeads) +
                                    ")] when keyAntiquantMode is per-tensor-head mode";
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(fiaInfo.opName, "key_antiquant_scale", actualShape.c_str(),
                                                  reasonMsg.c_str());
            return ge::GRAPH_FAILED;
        }
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerTokenHeadMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_3) {
        if (keyAntiquantScaleTensorShape.GetDim(DIM_NUM_0) != batchSize ||
            ...
            keyAntiquantScaleTensorShape.GetDim(DIM_NUM_2) < seqLength) {
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg = "The shape of keyAntiquantScale must be [B(" + std::to_string(batchSize) + "), N(" +
                                    std::to_string(numKeyValueHeads) + "), >=S(" + std::to_string(seqLength) +
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerTokenPAMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_2) {
        ...
        if (keyAntiquantScaleTensorShape != expectedShape) {
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg = "The shape of keyAntiquantScale must be [blockNum(" + std::to_string(blockNum) +
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerTokenHeadPAMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_3) {
        ...
        if (keyAntiquantScaleTensorShape != expectedShape) {
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg = "The shape of keyAntiquantScale must be [blockNum(" + std::to_string(blockNum) +
                                    "), N(" + std::to_string(numKeyValueHeads) + "), blockSize(" +
...
ge::graphStatus DequantChecker::CheckKScaleShapeForPerTokenGroupMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (keyAntiquantScaleTensorDimNum == DIM_NUM_5) {
        if (keyAntiquantScaleTensorShape.GetDim(DIM_NUM_0) != 1 ||
            ...
            std::string actualShape = ToStringRaw(keyAntiquantScaleTensorShape);
            std::string reasonMsg = "The shape of keyAntiquantScale must be [1, B(" + std::to_string(batchSize) +
                                    "), N(" + std::to_string(numKeyValueHeads) + "), >=S(" + std::to_string(seqLength) +
                                    "), D/32(" + std::to_string(headDim / 32) +
...
ge::graphStatus DequantChecker::CheckVScaleShapeForPerTokenMode(const FiaTilingInfo &fiaInfo) const
{
    ...
    if (valueAntiquantScaleTensorDimNum == DIM_NUM_2) {
        ...
        if (valueAntiquantScaleTensorShape.GetDim(DIM_NUM_0) != batchSize ||
            ...
            std::string reasonMsg =
                "The shape of valueAntiquantScale must be [B(" + std::to_string(batchSize) + "), >=S(" +
    ...
    if (valueAntiquantScaleTensorDimNum == DIM_NUM_3) {
        ...
        if (valueAntiquantScaleTensorShape.GetDim(DIM_NUM_0) != 1 ||
            ...
            std::string actualShape = ToStringRaw(valueAntiquantScaleTensorShape);
            std::string reasonMsg =
                "The shape of valueAntiquantScale must be [1, B(" + std::to_string(batchSize) + "), >=S(" +
                std::to_string(seqLength) +
