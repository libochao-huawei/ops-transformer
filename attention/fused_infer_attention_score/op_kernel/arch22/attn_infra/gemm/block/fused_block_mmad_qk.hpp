...
struct BlockMmad<MmadAtlasA2FAIQK<PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_, BNBD_FLAG_>, L1TileShape_, L0TileShape_, AType_,
                 BType_, CType_, BiasType_, TileCopy_, TileMmad_> {
public:
    // Type Aliases
    using DispatchPolicy = MmadAtlasA2FAIQK<PAGED_CACHE_FLAG_, ENABLE_UNIT_FLAG_, BNBD_FLAG_>;
                                ...
                                uint32_t KVStackLen = 512, uint32_t l1BufAddrStart = 0)
    {
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            l1BTensor[i] = resource.l1Buf.template GetBufferByByte<ElementB>(
                l1BufAddrStart + L1TileShape::M * kDyn * sizeof(ElementA) + nDyn * kDyn * sizeof(ElementB) * i);
    ...
    __aicore__ inline void getBlockShape(GemmCoord &actualShape, uint32_t nL1Index, uint32_t nL1Loop,
    ...
    __aicore__ inline void getKVOffset(AscendC::GlobalTensor<int32_t> &gBlockTable, uint64_t &kOffset, uint32_t nowNIdx,
                                       uint32_t startOffset, uint32_t strideKV, uint32_t blockSize, uint32_t embed,
                                       uint64_t keyBnStride)
    {
        ...
        if (keyBnStride != 0) {
            if constexpr (std::is_same_v<LayoutB, layout::nZ>) {
                ...
                kOffset = blockTableId * keyBnStride + startOffset * embed;
            } else {
        ...
        for (uint32_t nL1Idx = 0; nL1Idx < nL1Loop; ++nL1Idx) {
            ...
            if constexpr (PAGED_CACHE_FLAG_) {
                ...
                while (kvL1Len < l1NResDynamic) {
                    ...
                    getKVOffset(gBlockTable, gBOffset, nowNIdx, blockStartOffset, strideKV, blockSize, embed,
                                keyBnStride);
            ...
            for (uint32_t mL0Idx = 0; mL0Idx < mL0Loop; mL0Idx++) {
                ...
                for (uint32_t kL0Idx = 0; kL0Idx < kL0Loop; kL0Idx++) {
                    ...
                    tileMmad(l0CTensor[l0CPingPongFlag], l0ATensor[l0ABPingPongFlag], l0BTensor[l0ABPingPongFlag],
                             mL0Align, nActual, kL0Actual, initMmadFlag);
