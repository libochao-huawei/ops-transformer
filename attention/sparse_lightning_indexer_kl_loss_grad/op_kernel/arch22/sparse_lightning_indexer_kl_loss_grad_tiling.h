...
/*!
 * \file sparse_lightning_indexer_kl_loss_grad_tiling.h
 * \brief
 */
#ifndef SPARSE_LIGHTNING_INDEXER_KL_LOSS_GRAD_TILING_H
#define SPARSE_LIGHTNING_INDEXER_KL_LOSS_GRAD_TILING_H
...
public:
    ...
    int64_t get_cmpRatio() const {return cmpRatio;}
    void set_cmpRatio(int64_t cmpRatioParam) {this->cmpRatio = cmpRatioParam;}

    int64_t get_hasSoftmaxInput() const {return hasSoftmaxInput;}
    void set_hasSoftmaxInput(int64_t hasSoftmaxInputParam) {this->hasSoftmaxInput = hasSoftmaxInputParam;}
...
class SLIKLLossGradMultiCoreParams {
public:
    uint32_t coreNum;
    uint32_t rsvd;
...
class SLIKLLossGradInitOutputParams {
public:
...
class SLIKLLossGradVecApiParams {
public:
    SoftMaxTiling softmaxYTilingData;
    SoftMaxTiling simpleSoftmaxPTilingData;
};

class SparseLightningIndexerKLLossGradTilingData {
public:
    SLIKLLossGradBaseParams baseParams;
    SLIKLLossGradMultiCoreParams multiCoreParams;
    SLIKLLossGradInitOutputParams initOutputParams;
    SLIKLLossGradVecApiParams vectorParams;
};
