...
Formula source: ops-transformer-testkit/core/attention_cost.py,
core/metrics.py and data/flash_attn.py (operator-owned cost model).
MFU = 2*N1*(D+DV)*visible_pairs / seconds / peak_FLOPS.
MBU = (unique visible Q/K/V reads + complete Out/LSE writes) / seconds / peak_BW.
FP16/BF16 tensors use 2 bytes/element; LSE uses 4. Mask modes 0/3/4 are supported.
These are logical costs, excluding metadata, mask/block tables, workspace,
block padding, repeated loads and cross-batch cache sharing. Use main operator
device duration, excluding metadata preparation, for meaningful utilization.
...
API_NAMES = {
    "flash_attn_ttk_ops.flash_attn_ttk",
    "torch.ops.cann_ops_transformer.flash_attn",
}
METRIC_FIELDS = (
    "metrics_duration_us",
    "theoretical_flops",
...
def visible_counts(q, k, left, right):
    """Right-aligned window: (pairs, nonempty rows, unique columns), O(1).

    Piecewise arithmetic-series equivalent of reference metrics._window_pairs.
    None denotes an unbounded window side.
    """
    if not q or not k:
        return 0, 0, 0
    left = max(q, k) if left is None else left
    right = max(q, k) if right is None else right
    base = k - q
    clamp = lambda x: max(0, min(x, q))
    points = sorted({0, q, clamp(left - base), clamp(q - right), clamp(-base - right)})
    pairs = 0
    for start, end in zip(points, points[1:]):
        if base + start + right < 0:
            continue
        pos = base + start
        low, high = max(0, pos - left), min(k - 1, pos + right)
        slope = int(pos - left < 0) - int(pos + right > k - 1)
        count = end - start
        pairs += count * (high - low + 1) + slope * count * (count - 1) // 2
    return pairs, min(q, k + right), min(k, q + left)


def theoretical_cost(row):
    """Decode a FlashAttn TTK case without executing its Python API."""
    ...
    if (
        ...
        raise ValueError("expected attributes dict and Q/K/V shapes")
    if not isinstance(dtypes, (tuple, list)) or not dtypes:
        raise ValueError("missing tensor_dtypes")
    qkv_dtypes = dtypes * 3 if len(dtypes) == 1 else dtypes[:3]
    if len(qkv_dtypes) != 3 or any(
        x not in ("float16", "bfloat16", "fp16", "bf16") for x in qkv_dtypes
    ):
        raise ValueError("only FP16/BF16 Q/K/V are supported")
    q, k, v = shapes[:3]
    lq = attrs.get("layout_q", "BSND")
    lk = attrs.get("layout_kv", "BSND")
    ranks = {"BNSD": 4, "BSND": 4, "TND": 3, "PA_BBND": 4, "PA_BNBD": 4, "PA_NZ": 5}
    if lq not in ("BNSD", "BSND", "TND") or lk not in ranks:
        raise ValueError(f"unsupported layouts: {lq}/{lk}")
    for name, shape, layout in (("Q", q, lq), ("K", k, lk), ("V", v, lk)):
        if not isinstance(shape, (tuple, list)) or len(shape) != ranks[layout]:
            raise ValueError(f"{name}: invalid rank for {layout}")
        if any(type(x) is not int or x < 0 for x in shape):
            raise ValueError(f"{name}: expected concrete nonnegative shape")
    n1, d = q[1 if lq in ("BNSD", "TND") else 2], q[-1]
    n2 = k[2 if lk in ("BSND", "PA_BBND") else 1]
    kd = k[2] * k[4] if lk == "PA_NZ" else k[-1]
    dv = v[2] * v[4] if lk == "PA_NZ" else v[-1]
    geometry = lambda s: (s[0], s[1], s[3], s[4]) if lk == "PA_NZ" else s[:-1]
    if geometry(k) != geometry(v) or kd != d or min(n1, n2, d, dv) <= 0 or n1 % n2:
        raise ValueError("inconsistent Q/K/V heads or dimensions")
    q_capacity = capacity(q, lq, "q", attrs)
    q_used = used_lengths(attrs, "q", q_capacity)
    # Tensor values must be explicit: never infer active lengths from names or
    # silently assume all tokens are used when a sequence tensor was provided.
    for index, field in (
        (4, "cu_seqlens_q"),
        (5, "cu_seqlens_kv"),
        (6, "seqused_q"),
    ...
    if lk.startswith("PA_"):
        value = attrs.get("seqused_kv_values")
        if value is not None:
            kv_used = lengths(value, "seqused_kv_values")
        else:
            # PA storage shape is a block pool, never a per-batch token length.
            size = attrs.get("max_seqlen_kv")
            if type(size) is not int or size < 0:
                raise ValueError(
                    "PA requires seqused_kv_values or explicit max_seqlen_kv"
                )
            kv_used = [size] * len(q_capacity)
    else:
        kv_used = used_lengths(attrs, "kv", capacity(k, lk, "kv", attrs))
    if len(q_used) != len(kv_used):
        raise ValueError("Q/KV batch counts differ")
    mode = attrs.get("mask_mode", 0)
    if mode == 0:
        left = right = None
        if len(shapes) > 9 and shapes[9] is not None:
            raise ValueError(
                "explicit mask in mode 0 requires mask values to count visible pairs"
            )
    elif mode == 3:
        left, right = None, 0
    elif mode == 4:
        left, right = attrs.get("win_left", -1), attrs.get("win_right", -1)
        if type(left) is not int or type(right) is not int:
            raise ValueError("window bounds must be integers")
        left, right = None if left < 0 else left, None if right < 0 else right
    else:
        raise ValueError(f"mask_mode={mode}: only 0/3/4 are supported")
    counts = [visible_counts(sq, sk, left, right) for sq, sk in zip(q_used, kv_used)]
    pairs, rows, cols = (sum(x[i] for x in counts) for i in range(3))
    output = sum(q_capacity)
    flops = 2 * n1 * (d + dv) * pairs
    total_bytes = 2 * (n1 * d * rows + n2 * (d + dv) * cols + n1 * dv * output)
    if attrs.get("return_softmax_lse", False):
        total_bytes += 4 * n1 * output
