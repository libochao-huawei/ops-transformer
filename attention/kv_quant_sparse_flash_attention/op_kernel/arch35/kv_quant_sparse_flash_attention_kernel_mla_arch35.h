...
/*!
 * \file kv_quant_sparse_flash_attention_kernel_mla_arch35.h
 * \brief
 */

#ifndef KV_QUANT_SPARSE_FLASH_ATTENTION_KERNEL_MLA_ARCH35_H
#define KV_QUANT_SPARSE_FLASH_ATTENTION_KERNEL_MLA_ARCH35_H
...
#include "kv_quant_sparse_flash_attention_service_vector_mla_arch35.h"
            ...
            for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; ++s2LoopCount) {
                if (notLastTwoLoop) {
                    ...
                    if ASCEND_IS_AIC {
                        ...
                        this->vecBlock.ProcessVec0(this->l1RightBuffers.Get(), this->v0ResGmBuffers.Get(), runInfo1,
                                                   this->constInfo);
                    }
                } else {
                    if ASCEND_IS_AIV {
    ...
    int64_t bnIndex, int64_t gS1Index, RunParamStr &runParam)
{
    ...
    if constexpr (IS_SPLIT_G) {
        runParam.goIdx = (aicIdx % 2 == 0) ?
                             0 :
                             64; // N1=128场景，相邻cube核处理一个s1，第一个cube核承担0-63行g，第二个cube核承担后64行g
    } else {
        runParam.goIdx = 0;
    }
}
...
#endif // KV_QUANT_SPARSE_FLASH_ATTENTION_KERNEL_MLA_ARCH35_H
