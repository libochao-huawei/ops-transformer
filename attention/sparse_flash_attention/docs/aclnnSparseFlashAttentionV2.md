...
int InitializeTensors(TensorResources& resources) {
    ...
    std::vector<int64_t> sinksShape = {1};
    ...
    int64_t sinksShapeSize = GetShapeSize(sinksShape);
    ...
    std::vector<float> sinksHostData(sinksShapeSize, 1);
    ...
    // Create sinks aclTensor.
    ret = CreateAclTensor(sinksHostData, sinksShape, &resources.sinksDeviceAddr, 
                         aclDataType::ACL_FLOAT, &resources.sinksTensor);
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
      return ret;
    }
...
int ExecuteSparseFlashAttention(TensorResources& resources, aclrtStream stream, 
    ...
    int ret = aclnnSparseFlashAttentionV2GetWorkspaceSize(resources.queryTensor, resources.keyTensor, resources.valueTensor, resources.sparseIndicesTensor, nullptr, nullptr, nullptr, resources.queryRopeTensor, resources.keyRopeTensor,
                                                    resources.sinks, scaleValue, sparseBlockSize, layoutQuery, layoutKv, sparseMode, preTokens,
    ...
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnSparseFlashAttentionV2 failed. ERROR: %d\n", ret);
        return ret;
    }
...
void CleanupResources(TensorResources& resources, void* workspaceAddr, 
    ...
    if (resources.sinksTensor) {
      aclDestroyTensor(resources.sinksTensor);
    }
    ...
    if (resources.sinksDeviceAddr) {
      aclrtFree(resources.sinksDeviceAddr);
    }
