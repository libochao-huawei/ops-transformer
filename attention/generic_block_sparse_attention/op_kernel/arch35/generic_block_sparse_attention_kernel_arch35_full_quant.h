...
#ifndef GENERIC_BLOCK_SPARSE_ATTENTION_KERNEL_ARCH35_FULL_QUANT_H
#define GENERIC_BLOCK_SPARSE_ATTENTION_KERNEL_ARCH35_FULL_QUANT_H
...
template <class BlockMmadQK, class EpilogueOnlineSoftmax, class BlockMmadPV, class EpilogueRescaleO, Format qFormat,
          Format kvFormat>
class GbsaFullQuantKernelArch35 {
public:
    ...
    __aicore__ inline GbsaFullQuantKernelArch35() {}

    __aicore__ inline void operator()(GbsaFullQuantKernelParamsArch35 const &params)
    {
        __gm__ GenericBlockSparseAttn::GenericBlockSparseAttentionTilingData *gbsaTilingData =
            reinterpret_cast<__gm__ GenericBlockSparseAttn::GenericBlockSparseAttentionTilingData *>(params.tiling);
        FetchBaseShapeInfo(gbsaTilingData, params.metaData);
        CalcOnChipBufTileInfo(gbsaTilingData);
        ...
        if (!GsaFd::ValidateMetadata(meta, gbsaTilingData, coreNum)) {
            return;
        }
        const bool fdEnabled = gbsaTilingData->fdStaticEnabled != 0U &&
        ...
        gPartialLse.SetGlobalBuffer((__gm__ float *)(params.workSpace + gbsaTilingData->fdPartialLseOffset));
        AscendC::GlobalTensor<float> gPartialO;
        gPartialO.SetGlobalBuffer((__gm__ float *)(params.workSpace + gbsaTilingData->fdPartialOOffset));
        ...
        for (uint32_t taskIdx = taskLoopStart; taskIdx < taskLoopEnd; taskIdx += taskLoopStep) {
            ...
            for (uint32_t kvBlockIdx = 0; kvBlockIdx < kvSLoopNum + PRE_LAUNCH; kvBlockIdx++) {
                if (kvBlockIdx < kvSLoopNum) {
                    ...
                    blockMmadQK(gmKTensorTla, ubSTensorTla, gIdentityIdx, actualBlockShapeQK, 0, blockSize_, blockSize_,
                                blockSize_, 1, 1, prefixSumL0AStages, prefixSumL0BStages, mm1ToSmFlag);
                ...
                if (kvBlockIdx >= PRE_LAUNCH) {
                    ...
                    blockMmadPV(gmVTensorTla, ubOTmpTensorTla, gIdentityIdx, actualBlockShapePV, kvBlockIdxDe,
                                blockSize_, blockSize_, blockSize_, 1, kvSLoopNum, prefixSumL0AStages,
                                prefixSumL0BStages, smToMm2Flag, mm2ToReFlag);
#endif
#ifdef __DAV_VEC__
                    Arch::CrossCoreFlag mm2ToReFlag(Mm2ToReFlagId);
                    ...
                    if (isFdPartial) {
                        epilogueRescaleO.ProcessPartial(gmOTensorTla, gmLseTensorTla, actualBlockShapePV, curTileMod,
                                                        ...
                                                        gPartialLse, fdPartialTaskId, gbsaTilingData->fdLseSubStride);
                    } else {
        ...
        if (fdEnabled) {
            ...
            combine(meta, gbsaTilingData, gPartialLse, gPartialO, gO, gLse, gCuSeqLengths, gSequsedQ, hasSequsedQ);
#endif
        }
    }
...
private:
    ...
    __aicore__ inline uint64_t CalcCrossMm1Mm2PrefixSumL0ABStages(uint32_t kvBlockIdx, uint32_t singleMm1L0Stages,
...
#endif // GENERIC_BLOCK_SPARSE_ATTENTION_KERNEL_ARCH35_FULL_QUANT_H
