...
#ifndef DISPATCH_FFN_COMBINE_BF16_KERNEL_A2_HPP
#define DISPATCH_FFN_COMBINE_BF16_KERNEL_A2_HPP
...
>
class DispatchFFNCombineBF16KernelA2 {
public:
    ...
    struct Params {
        ...
        Mc2Tiling::MoeInitRoutingV2TilingData moeInitRoutingQuantV2TilingData;
        //--------------
        ...
        Params(
            ...
            Mc2Tiling::MoeInitRoutingV2TilingData moeInitRoutingQuantV2TilingData_,
            ...
            ubMoveNum(ubMoveNum_), symmetricPtr(symmetricPtr_),
            moeInitRoutingQuantV2TilingData(moeInitRoutingQuantV2TilingData_)
        {
            moeInitRoutingQuantV2TilingData.vbsComputeParamsOp =
                moeInitRoutingQuantV2TilingData_.vbsComputeParamsOp;
    ...
    // Methods
    CATLASS_DEVICE
    DispatchFFNCombineBF16KernelA2(Params const &params)
    {
        if ASCEND_IS_AIC {
        ...
        if ASCEND_IS_AIV {
            ...
            serverId_ = params.rank / SERVER_RANK_SIZE_A2;
        }
    ...
    CATLASS_DEVICE
    ~DispatchFFNCombineBF16KernelA2()
    {
    }
...
private:
    ...
    {
#ifdef HCCL_COMM
        qp_info_ = (__gm__ HcclAiRMAInfo*)(((__gm__ HcclA2CombineOpParam*)shmem.WinContext_)->aiRMAInfo);
#else
    ...
    )
    {
        ...
        for (uint32_t processIndex = 0; processIndex < processCount; ++processIndex) {
            uint32_t curProcessNum = (processIndex == processCount - 1) ?
                elemNum - ubMoveNum * (processCount - 1) : ubMoveNum;
    ...
    )
    {
        ...
        if (static_cast<int32_t>(params.rank) == dstEpIdx) {
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);
        ...
        if (dstServerId == serverId_) {
            ...
            remoteFlag.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t*>(
                ...
                + (params.rank * params.expertPerRank + groupIdx) * sizeof(uint32_t)));
            flagUb.SetValue(0, 1);
        ...
        // ── 路径 C：跨 server（批量 RDMA 数据 + flag RDMA）─────────────
        {
            ...
            GM_ADDR remoteFlagAddr = reinterpret_cast<GM_ADDR>(
                ...
                + static_cast<uintptr_t>(params.rank * params.expertPerRank + groupIdx) * sizeof(uint32_t));
            AIVRDMAPostSend(
    ...
    )
    {
        ...
        if (srcEpIdx == static_cast<int32_t>(params.rank)) return;
    ...
    void GetCumsumForMMAIV(AscendC::GlobalTensor<int32_t> &tokenPerExpert,
        AscendC::GlobalTensor<int32_t> &result, uint32_t expertPerRank, uint32_t EP)
    {
...
#define U16(x) static_cast<uint16_t>(x)
        ...
        for (uint32_t i = 1; i < EP; ++i) {
            AscendC::Add(tmpBuffer1[i * expertPerRankAligned],
        ...
        uint32_t expertPerRank, uint32_t rankId, uint32_t EP)
    {
        ...
        if (coreIdx < EP) {
            ...
            DataCacheCleanAndInvalid<int32_t, CacheLine::SINGLE_CACHE_LINE,
                DcciDst::CACHELINE_OUT>(result[coreIdx * 16]);
            __asm__ __volatile__("");
        }
    }
    ...
    void GMM1(Params const &params)
    {
        ...
        // Wait for AIV to finish cumsum for matmul
        AscendC::CrossCoreWaitFlag<0x2>(syncgmmIdx / CROSS_CORE_FLAG_MAX_SET_COUNT);
        syncgmmIdx++;

        for (uint32_t groupIdx = 0; groupIdx < params.expertPerRank; ++groupIdx) {
            uint32_t currentM = cumsumMM(tokenPerExpertLayout(params.EP - 1, params.rank, groupIdx));
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += coreNum) {
                ...
                if (currentM > 0) {
                    if constexpr (std::is_same_v<ElementA, int8_t>) {
                        // 每个expert一组scale
                        int64_t gmOffsetS = groupIdx * params.problemShape.n() +
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.expertPerRank; ++groupIdx) {
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += coreNum) {
                ...
                if (currentM > 0) {
                    if constexpr (std::is_same_v<ElementA, int8_t>) {
                        // 每个expert一组scale
                        int64_t gmOffsetS = groupIdx * n2 + blockCoord.n() * L1TileShape::N;
                        blockMmad(
    ...
    CATLASS_DEVICE
    void CrossRankSyncAndlocalTokenPerExpertAllGatherAndGetSumPreRankV2(
        Params const &params, int64_t localTokenPerExpertOffset)
    {
        ...
        for (int32_t dstEpIdx = coreIdx, dstServerId = 0; dstEpIdx < params.EP; dstEpIdx += coreNum) {
            ...
            GM_ADDR localDstPeermemPtr = shmem.windowsOutAddr() +
                peermemInfo.offsetPeerTokenPerExpert +
            ...
            if (dstServerId == serverId_) {
                ...
                AIVRDMAPostSend((GM_ADDR)localDstAddress.GetPhyAddr(),
                    (GM_ADDR)dstAddress.GetPhyAddr(), dstEpIdx,
                    numPerCore * sizeof(int32_t),
                    qp_info_, rdmaUbLocal, rdmaUbLocalHead);
            }
        }
        ...
        for (int32_t dstEpIdx = coreIdx; dstEpIdx < params.EP; dstEpIdx += coreNum) {
            ...
            // 只有 dstEpIdx == params.rank 的 core 才拥有本 rank 自身的 token 分布，
            // 只允许该 core 写 preSumBeforeRank，避免其他 core 覆盖正确数据（写竞争修复）
            if (static_cast<uint32_t>(dstEpIdx) == params.rank) {
            ...
            for (int32_t i = 0, j = 0, prevSum = 0; i < (params.rank + 1) * params.expertPerRank; i++) {
    ...
    void ResetTokenPerExpert(int32_t num)
    {
        ...
        AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(EVENT_ID0);
        // 清零 winIn tokenPerExpert 区域（供 V2/V4 Stage 接收轮询使用）
        AscendC::DataCopy(tokenPerExpert, tmp, num);
        AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);
        // 清零 winOut tokenPerExpert 区域（供 V3 Stage1 写目标 + rank-0 轮询使用）
    ...
    void UpdateAicFlags(const Params &params)
    {
        ...
        AscendC::LocalTensor<float> dstValueBuffer =
            resource.ubBuf.template GetBufferByByte<float>(flagBufferSize);
    ...
    void DispatchAndCombine(Params const &params)
    {
        ...
        int64_t localTokenPerExpertOffset = peermemInfo.offsetPeerTokenPerExpert +
            tokenPerExpertLayout(params.rank, 0, 0) * sizeof(int32_t);
        // Place the entire communication matrix in peermem
        GM_ADDR localTokenPerExpert = shmem.windowsOutAddr() + localTokenPerExpertOffset;
        uint32_t expandedRowIdxOffset = AlignUp(params.problemShape.m(), 256) *
            params.topK * sizeof(int32_t);

        moe_init_routing_v2<ElementA>(reinterpret_cast<GM_ADDR>(params.ptrA),
            params.expertIdx, shmem.windowsOutAddr() + peermemInfo.offsetA,
        ...
        CrossRankSyncAndlocalTokenPerExpertAllGatherAndGetSumPreRankV2(
            params, localTokenPerExpertOffset);
        ...
        if (coreIdx == 0) {
            CopyGMToGM(ExpertTokenNums,
                cumsumMM[tokenPerExpertLayout(params.EP - 1, params.rank, 0)],
        ...
        // prevGroupSum2:   params.rank 的 peer mem 中已接收 token 的基准偏移（所有 core 共享）
        ...
        for (int32_t groupIdx = 0; groupIdx < params.expertPerRank; ++groupIdx) {
            // params.rank 本轮专家组接收的 token 总数（所有 source rank 之和）
            uint32_t currentRankM = static_cast<uint32_t>(
                cumsumMM(tokenPerExpertLayout(params.EP - 1, params.rank, groupIdx)));
            ...
            for (int32_t dstEpIdx = static_cast<int32_t>(coreIdx);
                dstEpIdx < params.EP; dstEpIdx += static_cast<int32_t>(coreNum)) {
                uint32_t rowStart = prevGroupSum1 +
                    (params.rank == 0 ? 0u : static_cast<uint32_t>(
                        cumsumMM(tokenPerExpertLayout(params.rank - 1, dstEpIdx, groupIdx))));
                ...
                if (rowStart < params.maxOutputSize) {
                    uint32_t rows = tokenPerExpert(tokenPerExpertLayout(params.rank, dstEpIdx, groupIdx));
            ...
            for (int32_t srcEpIdx = static_cast<int32_t>(coreIdx);
                ...
                uint32_t rowStart2 = prevGroupSum2 +
                    (srcEpIdx == 0 ? 0u : static_cast<uint32_t>(
                        cumsumMM(tokenPerExpertLayout(srcEpIdx - 1, params.rank, groupIdx))));
                if (rowStart2 < params.maxOutputSize) {
                    uint32_t rows2 = static_cast<uint32_t>(
                        tokenPerExpert(tokenPerExpertLayout(srcEpIdx, params.rank, groupIdx)));
            ...
            prevGroupSum2 += currentRankM;  // params.rank 累计收到的 token 数
        ...
        if (get_subblockid() == 0) {
            ...
            kernelMoeTokenUnpermuteOp.Init(shmem() + peermemInfo.offsetD,
                workspaceInfo.expandedRowIdx, params.probs,
    ...
    void CombineV2(Params const &params, BlockEpilogue2 & blockEpilogue)
    {
...
#ifdef HCCL_COMM
        AscendC::LocalTensor<uint64_t> rdmaUbLocal =
            resource.ubBuf.template GetBufferByByte<uint64_t>(128 * 1024);
        AscendC::LocalTensor<uint32_t> rdmaUbLocalHead =
        ...
        icache_preload(8);
        for (uint32_t groupIdx = 0; groupIdx < params.expertPerRank; ++groupIdx) {
            uint32_t currentExpertM = cumsumMM(tokenPerExpertLayout(params.EP - 1, params.rank, groupIdx));
            ...
            blockScheduler.Update(inGroupProblemShape, MakeCoord(L1TileShape::M, L1TileShape::N));
            uint32_t coreLoops = blockScheduler.GetCoreLoops();
            uint32_t startLoopIdx = ((aicCoreIdx < startCoreIdx) ?
                (aicCoreIdx + aicCoreNum) : aicCoreIdx) - startCoreIdx;
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += aicCoreNum) {
                ...
                for (int32_t cur_row = 0; cur_row < aiv_m_rows; cur_row++) {
                    ...
                    blockEpilogue(gmC2, realTileCoord, realTileShape, groupIdx,
...
private:
    ...
    struct PeermemInfo {
        ...
        int64_t offsetD;
        // EP × expertPerRank × 4 bytes；每个 sender rank 占 expertPerRank 个
        // uint64_t slot，存放 epoch 到达标志
        int64_t offsetFlag;
        CATLASS_DEVICE
        PeermemInfo() {}
...
#endif // DISPATH_FFN_COMBINE_KERNEL_A2_HPP