...
__aicore__ inline void MoeDistributeDispatchA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::InitUBBuffer()
{
    ...
    uint32_t expertToServerCntSize =
        RoundUp(axisBS_ * serverNum * static_cast<uint32_t>(sizeof(int32_t)), UB_32B_ALIGN);
...
__aicore__ inline void MoeDistributeDispatchA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::CreateInnerReduceInfo()
{
    ...
    LocalTensor<int32_t> combineReduceInfo =
        tBuf.GetWithOffset<int32_t>(moeExpertNumInServer_ * realBS, baseBuffOffset);
    ...
    LocalTensor<int32_t> tokenOffset =
        tBuf.GetWithOffset<int32_t>(RoundUp(realBS * axisK_, BITS32_PER_BLOCK), baseBuffOffset);
        ...
        for (uint32_t tokenIdx = 0; tokenIdx < validCnt; tokenIdx++) {
            ...
            for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
                ...
                if (expId >= currServerExpBegin && expId < currServerExpEnd) {
                    ...
                    combineReduceInfo(expIdInServer * realBS + tokenOffset(tokenId * axisK_ + expIdx)) =
                        invalidInfoData;
...
__aicore__ inline void MoeDistributeDispatchA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::ReorderTokens()
{
    ...
    LocalTensor<float> tokenCastLt = tBuf.GetWithOffset<float>(
        axisH_ * sendTokenNum, RoundUp(TBUF_TEMP_OFFSET + singleTokenUBSize * maxTokenNumInUB, B32_PER_BLOCK));
    ...
    for (uint32_t batchIndex = 0; batchIndex < batchNum; batchIndex++) {
        ...
        if constexpr (DynamicQuant || StaticQuant) {
            ...
            DataCopyExtParams tokenCopyParams{static_cast<uint16_t>(currentTokenNum),
                                              static_cast<uint32_t>(axisH_ * sizeof(XType)), 0,
                                              static_cast<uint32_t>(tokenGapInStruct_), 0};
            DataCopyPadExtParams<uint8_t> tokenPadParams;
            DataCopyPad(tokenTempTensorU8_, xGMTensorU8_[startTokenId * tokenLenInStruct_], tokenCopyParams,
                        tokenPadParams);
        }
        // Expert进行拷贝
        ...
        DataCopyExtParams weightCopyParams{static_cast<uint16_t>(currentTokenNum),
                                           static_cast<uint16_t>(realLenInStruct_), 0,
                                           static_cast<uint16_t>(infoGapInStruct_), 0};
...
template <TemplateMC2TypeA2layeredAicpuClass>
__aicore__ inline void MoeDistributeDispatchA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::QuantProcess(
    ...
    uint32_t localTokenIdx, uint32_t globalTokenIdx, uint32_t dstServerId)
{
    ...
    LocalTensor<uint8_t> tokenTempTensorU8_ = tBuf.GetWithOffset<uint8_t>(tokenUbSize_, TBUF_TEMP_OFFSET);
    SyncFunc<AscendC::HardEvent::S_MTE3>();
...
MoeDistributeDispatchA2LayeredAicpu<TemplateMC2TypeA2layeredAicpuFunc>::ConstructDataAndFlagBatchWriteInfo()
{
    // 计算当前core要处理的server
...
#pragma unroll 8
    ...
    LocalTensor<uint8_t> localUB = tBuf.GetWithOffset<uint8_t>(tokenUbSize / sizeof(uint8_t), TBUF_TEMP_OFFSET);
    LocalTensor<float> localUBfloat = tBuf.GetWithOffset<float>(tokenUbSize / sizeof(float), TBUF_TEMP_OFFSET);
    LocalTensor<int32_t> localUBint32 = tBuf.GetWithOffset<int32_t>(tokenUbSize / sizeof(int32_t), TBUF_TEMP_OFFSET);
