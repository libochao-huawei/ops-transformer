...
public:
                                                          ...
                                                          uint32_t &aivLoopNum)
    {
        ...
        tileCalCntM = quantUbSize / this->alignN;
    ...
    __aicore__ inline void MatmulAllReduceQuantReduceSumSplitMAndN(int32_t &quantUbSize, uint32_t &needAivCoreNum,
                                     ...
                                     int64_t scaleAddrOffsetSplitMN, uint32_t blockCntSpiltMN)
    {
        ...
        tempBufferGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int8_t *>(this->tempBufferGM) + curBlockAddrOffset,
        ...
        if (((loopIdx == 0) && (this->splitMode == SPLIT_M_MATMUL_ALLREDUCE_INT8)) ||
            ...
            quantScale1Gm_.SetGlobalBuffer(reinterpret_cast<__gm__ T *>(this->quantScale1GM) + curScaleAddrOffset,
                                           curScaleCnt);
            quantScale2Gm_.SetGlobalBuffer(reinterpret_cast<__gm__ T *>(this->quantScale2GM) + curScaleAddrOffset,
                                           curScaleCnt);
        }
    }
                                   ...
                                   uint32_t blockCntSpiltMN)
    {
        ...
        for (uint32_t i = 1; i < this->rankCnt; i++) {
            ...
            int64_t offset = static_cast<int64_t>(i) * static_cast<int64_t>(this->tileRankM) *
                                static_cast<int64_t>(this->N);
            DataCopyPad(allToAllAddLocal, tempBufferGm_[offset], copyParamsCurBlock,
        ...
        int64_t outOffset = static_cast<int64_t>(rank_id) * static_cast<int64_t>(this->tileRankM) *
                            static_cast<int64_t>(this->N);
        DataCopyPad(tempBufferGm_[outOffset], outLocal, copyOutParamsCurBlock);
    ...
    __aicore__ inline void SetParams(GM_ADDR tempBufferGM, GM_ADDR quantScale1GM, GM_ADDR quantScale2GM, uint32_t M,
                                     uint32_t N, typename HcclTypeSelector<commMode>::type &hccl_, uint32_t aivCoreNum,
                                     uint32_t &tileBlockCnt, uint32_t &tailBlockCnt, uint32_t &aivLoopNum,
                                     int64_t &blockAddrOffset, uint32_t &tailCalCntM, uint32_t &tileCalCntM,
                                     uint32_t &needAivCoreNum, uint32_t &blockNumPerRow, int32_t &nowQuantUbSize)
    {
        int32_t nowAlignN = Ceil(N * sizeof(int8_t), BYTE512_MATMUL_ALLREDUCE_INT8) * BYTE512_MATMUL_ALLREDUCE_INT8;
        nowQuantUbSize =
            (TOTAL_UBSIZE_MATMUL_ALLREDUCE_INT8 - static_cast<int32_t>(nowAlignN) * static_cast<int32_t>(sizeof(T)) *
        ...
        nowQuantUbSize = (nowQuantUbSize / nowAlignN) * nowAlignN;
        this->M = M;
        this->N = N;
        this->alignN = static_cast<uint32_t>(nowAlignN);
        ...
        if (nowQuantUbSize > this->tileRankM * nowAlignN) {
            nowQuantUbSize = this->tileRankM * nowAlignN;
        }

        if (nowQuantUbSize >= nowAlignN) { // 分核切M， 不切N
            this->splitMode = SPLIT_M_MATMUL_ALLREDUCE_INT8;
            this->MatmulAllReduceQuantReduceSumSplitM(nowQuantUbSize, blockAddrOffset, tileCalCntM,
                tailCalCntM, aivLoopNum);
        } else { // N超大，分核策略采取切MN
            this->splitMode = SPLIT_MN_MATMUL_ALLREDUCE_INT8;
            this->MatmulAllReduceQuantReduceSumSplitMAndN(nowQuantUbSize, needAivCoreNum, tileBlockCnt,
                tailBlockCnt, aivLoopNum, blockNumPerRow);
        }
    }
                                                    ...
                                                    typename HcclTypeSelector<commMode>::type &hccl_)
{
    ...
    op.SetParams(tempBufferGM, quantScale1GM, quantScale2GM, M, N, hccl_, aivCoreNum, tileBlockCnt, tailBlockCnt,
                 aivLoopNum, blockAddrOffset, tailCalCntM, tileCalCntM, needAivCoreNum, blockNumPerRow, nowQuantUbSize);
