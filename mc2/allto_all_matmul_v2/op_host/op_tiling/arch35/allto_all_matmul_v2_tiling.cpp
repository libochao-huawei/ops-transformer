...
#include "allto_all_matmul_v2_tiling_base.h"

namespace MC2Tiling {

bool AlltoAllMatmulV2TilingClass::IsCapable()
{
    return true;
}
...
bool AlltoAllMatmulV2TilingClass::CheckGroupAttr(const char *opName)
{
    ...
    if (groupLen == 0 || groupLen == MAX_GROUP_NAME_LEN) {
        OP_LOGE_FOR_INVALID_VALUE(opName, "group", std::to_string(groupLen).c_str(),
...
bool AlltoAllMatmulV2TilingClass::CheckTensorFormats(const char *opName)
{
    ...
    if (x1Fmt != ge::FORMAT_ND || x2Fmt != ge::FORMAT_ND) {
        OP_LOGE_FOR_INVALID_FORMATS_WITH_REASON(opName, "x1, x2",
                                                (Ops::Base::ToString(x1Fmt) + "," + Ops::Base::ToString(x2Fmt)).c_str(),
...
bool AlltoAllMatmulV2TilingClass::CheckQuantAndYDtypeAttr(const char *opName)
{
    ...
    if (x1QuantMode != static_cast<int64_t>(MX_QUANT_MODE) || x2QuantMode != static_cast<int64_t>(MX_QUANT_MODE)) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName, "x1_quant_mode, x2_quant_mode",
...
bool AlltoAllMatmulV2TilingClass::CheckMatmulDimsAndBounds(const char *opName)
{
    ...
    if (x1M == 0 || x1K == 0 || x2N == 0 || x2K == 0) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName, "x1, x2",
                                              ("x1=[" + std::to_string(x1M) + "," + std::to_string(x1K) + "], x2=[" +
                                               std::to_string(x2N) + "," + std::to_string(x2K) + "]")
                                                  .c_str(),
    ...
    if (x1K > K_MAX_VAL || x2K > K_MAX_VAL) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName, "x1_k, x2_k",
                                              ("x1_k=" + std::to_string(x1K) + ", x2_k=" + std::to_string(x2K)).c_str(),
...
bool AlltoAllMatmulV2TilingClass::CheckMatmulDivisibility(const char *opName)
{
    ...
    if (yM != x1M / worldSize || yN != x2N) {
        OP_LOGE_FOR_INVALID_SHAPE(opName, "y", ("[" + std::to_string(yM) + "," + std::to_string(yN) + "]").c_str(),
...
bool AlltoAllMatmulV2TilingClass::CheckBiasShape(const char *opName)
{
    ...
    if (biasShape->GetStorageShape().GetDimNum() != 1) {
        OP_LOGE_FOR_INVALID_SHAPEDIM(opName, "bias", std::to_string(biasShape->GetStorageShape().GetDimNum()).c_str(),
                                     "1D");
        return false;
    }
...
ge::graphStatus AlltoAllMatmulV2TilingClass::CheckOpInputInfo()
{
...
ge::graphStatus AlltoAllMatmulV2TilingClass::DoOpTiling()
{
        ...
        if (hccBufPtr != nullptr && static_cast<uint64_t>(*hccBufPtr) < needBytes) {
            ...
            OP_LOGE_WITH_INVALID_ATTR(
                ...
                ("no less than worldSize(" + std::to_string(worldSize_) + ") * mPerRank(" + std::to_string(m_) +
                 ...
                 std::to_string(scaleKGroups) + " * " + std::to_string(SCALE_LAST_DIM) + " * 8 bits)) / 8 + reserved(" +
                 std::to_string(HCCL_BUFFER_RESERVED_BYTES) + " bytes) = " + std::to_string(needBytes) + " bytes (" +
                 std::to_string(needMb) + " MB). Please export HCCL_BUFFSIZE=" + std::to_string(needMb) + " and retry")
                    .c_str());
            return ge::GRAPH_FAILED;
        }
    }
    ...
    OP_LOGI(opName, "win reuse: totalTiles=%lu, dataSlotBytes=%lu, scaleSlotBytes=%lu, fullWinBytes=%lu, slotNum=%lu",
    ...
    td->dumpInfo.peermemLayout.segments[0] = {0UL, dataRegionBytes, static_cast<uint8_t>(Utils::PEERMEM_SEG_DATA), {}};
...
ge::graphStatus AlltoAllMatmulV2TilingClass::DoLibApiTiling()
{
    return ge::GRAPH_SUCCESS;
}

uint64_t AlltoAllMatmulV2TilingClass::GetTilingKey() const
{
...
ge::graphStatus AlltoAllMatmulV2TilingClass::PostTiling()
{
