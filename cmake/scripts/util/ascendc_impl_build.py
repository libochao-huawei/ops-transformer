...
{}
    ...
    options.append("-I" + os.path.join(PYF_PATH, "..", "ascendc", "common", "tensor_api", "include"))
...
class AdpBuilder(opdesc_parser.OpDesc):
    ...
    def write_adapt(
        ...
        if (
            os.environ.get("BUILD_BUILTIN_OPP") != "1"
            and impl_path != ""
            and not self.is_pypto
        ):
    ...
    def _get_kernel_source(self: any, kernel_src_dir, src_file, dir_snake, dir_ex):
        ...
        if os.path.exists(src):
            return src
        src = os.path.join(
            kernel_src_dir, "..", "ascendc", dir_snake, "op_kernel", src_file
        )
        if os.path.exists(src):
            return src
        src_ex = os.path.join(
            kernel_src_dir, "..", "ascendc", dir_ex, "op_kernel", src_file
        )
        if os.path.exists(src_ex):
            return src_ex
...
if __name__ == "__main__":
    ...
    for ops_info in ops_infos:
        compile_opts = CompileOptions(
            compute_unit=args.compute_unit, pypto_ops=pypto_ops
        )
        write_scripts(
            cfgfile=ops_info, cfgs=rep_cfg, dirs=cfg_dir, compile_options=compile_opts
        )
