...
def collect_and_save_incremental(prof_folder, test_result_path):
    ...
    print(
        f"  [perf] {case_name} Task Duration: {row_data['Task Duration(us)']}us -> {perf_path}"
    )
    return row_data


def collect_all(
    test_result_path, is_compare=False, perf_golden_path="perf_golden.xlsx"
):
    ...
    if len(prof_folders) != valid_count:
        print(
            f"警告: PROF文件夹数量({len(prof_folders)})与有效用例数({valid_count})不一致"
        )
    ...
    for i in range(df_b.shape[0]):
        ...
        if row_data is not None:
            ...
            print(
                f"  [{prof_idx + 1}] {case_name} -> {prof} (Task Duration: {row_data['Task Duration(us)']}us)"
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--prof_folder", type=str, default=None, help="增量模式下指定 PROF 文件夹路径"
    )
        ...
        if args.prof_folder:
            ...
            prof_folders = sorted(
                [
                    d
                    for d in os.listdir(".")
                    if os.path.isdir(d) and d.startswith("PROF")
                ],
                key=lambda x: os.path.getmtime(x),
            )
            if not prof_folders:
