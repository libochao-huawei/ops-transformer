...
template <class BlockMmadQK, class BlockMmadPV, class EpilogueOnlineSoftmax, class EpilogueRescaleO,
          bool PAGED_CACHE_FLAG, uint32_t QUERY_LAYOUT, uint32_t KV_CACHE_LAYOUT>
...
public:
    ...
    __aicore__ inline BlockSparseAttentionKernel() {}
                                            ...
                                            AscendC::GlobalTensor<int32_t> selectNumGM)
    {
        ...
        uint32_t actDealtRow =
            (subCoreIdx == preActivateSubCoreNum - 1) ? (totalRowNumBlockMask - curStartRowIdx) : avgRowPerSubCore;
        if (subCoreIdx < preActivateSubCoreNum) {
            ...
            for (uint32_t i = 0; i < rowLoop; i++) {
                ...
                for (uint32_t j = 0; j < colLoop; j++) {
                    ...
                    int32_t maskOffset =
                        curStartRowIdx * maxKvBlockNum + curLoopRowOffset * maxKvBlockNum + curLoopColOffset;
                    AscendC::DataCopyPad(
                        maskPatternUbLocal[idxPingPongFlag], maskGM[maskOffset],
                        AscendC::DataCopyExtParams(actDealtRowCurLoop, actDealtColCurLoop * sizeof(int8_t),
                                                   (maxKvBlockNum - actDealtColCurLoop) * sizeof(int8_t),
                                                   (PRE_COL_TILE - actDealtColCurLoop32) / 32, 0),
                        AscendC::DataCopyPadExtParams<uint8_t>(true, 0, (actDealtColCurLoop32 - actDealtColCurLoop),
                                                               0));

                    AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(0);
                    AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(0);
                    AscendC::Cast(maskPatternHalfLocal, maskPatternUbLocal[idxPingPongFlag],
                                  AscendC::RoundMode::CAST_NONE, actDealtRowCurLoop * PRE_COL_TILE);
                    AscendC::PipeBarrier<PIPE_V>();
                    AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(idxPingPongFlag);
                    AscendC::Cast(maskPatternFloatLocal, maskPatternHalfLocal, AscendC::RoundMode::CAST_NONE,
                                  actDealtRowCurLoop * PRE_COL_TILE);
                    AscendC::PipeBarrier<PIPE_V>();
                    for (uint32_t k = 0; k < actDealtRowCurLoop; k++) {
                        if (actDealtColCurLoop32 != PRE_COL_TILE) {
                            AscendC::Duplicate(maskPatternFloatLocal[k * PRE_COL_TILE + actDealtColCurLoop32], (float)0,
                                               PRE_COL_TILE - actDealtColCurLoop32);
                        }
                        AscendC::PipeBarrier<PIPE_V>();
                        AscendC::CompareScalar(maskPatternInBitUbLocal[k * PRE_COL_TILE],
                                               maskPatternFloatLocal[k * PRE_COL_TILE], (float)1.0,
                                               AscendC::CMPMODE::GE, PRE_COL_TILE);
                        AscendC::PipeBarrier<PIPE_V>();
                        if (k == 0) {
                            AscendC::WaitFlag<AscendC::HardEvent::MTE3_V>(idxPingPongFlag);
                        }
                        AscendC::GatherMask(sparseIdxUbLocal[idxPingPongFlag][k * PRE_COL_TILE], maskIdxUbLocal,
                                            maskPatternInBitUbLocalUint32[k * PRE_COL_TILE / 4], false, (uint32_t)0,
                        ...
                        AscendC::DataCopyPad(
                            selectIdxGM[curStartRowIdx * maxKvBlockNum + curLoopRowOffset * maxKvBlockNum +
                                        k * maxKvBlockNum + rsvdCountPerRow[k]],
                            sparseIdxUbLocal[idxPingPongFlag][k * PRE_COL_TILE],
                            AscendC::DataCopyExtParams(1, rsvdCountPerRowCurColLoop[k] * sizeof(int32_t), 0, 0, 0));
                ...
                AscendC::DataCopyPad(selectNumGM[curStartRowIdx + curLoopRowOffset],
    ...
    __aicore__ inline void operator()(BlockSparseAttentionKernelParams const &params)
    {
        ...
        uint32_t qSeqlen = useUniformQSeqlen ?
                               maxQSeqlen :
        ...
        uint32_t kvSeqlen = useUniformKvSeqlen ?
                                maxKvSeqlen :
        ...
        for (uint32_t taskIdx = coreIdx; taskIdx < totalTaskNum; taskIdx += uint32_t(coreNum)) {
            while (taskIdx >= curTotalTaskNum) {
                ...
                // 根据useUniformQSeqlen标志位决定使用actualSeqLengths数组还是maxQSeqlen
                qSeqlen = useUniformQSeqlen ?
                              maxQSeqlen :
                              static_cast<uint32_t>(static_cast<int64_t>(gActualQseqlen.GetValue(curBatch)));
                // 根据useUniformKvSeqlen标志位决定使用actualSeqLengthsKv数组还是maxKvSeqlen
                kvSeqlen = useUniformKvSeqlen ?
                               maxKvSeqlen :
                               static_cast<uint32_t>(static_cast<int64_t>(gActualKvseqlen.GetValue(curBatch)));
            ...
            uint32_t qSBlockSize =
                (qXIdx == xBlockNum) ?
                    (qXInnerIdx == xTailNum / curQSBlockTile ? xTailNum - qXInnerIdx * curQSBlockTile :
                                                               curQSBlockTile) :
            ...
            for (uint32_t kvSIdx = 0; kvSIdx < kvSLoopNumTotal + preKVNum; kvSIdx += blockStackNum) {
                ...
                if (kvSIdx < kvSLoopNumTotal) {
                    ...
                    // online softmax
                    epilogueOnlineSoftmax(gP[gmOffsetP], gS[gmOffsetS], layOutP, layOutS, actualBlockShapeQK,
                                          (stackSeqCount == 0), 0, qSBlockSize, qNBlockSize, curStackTileMod,
                                          softmaxReady);
#endif
                }
                ...
                if (kvSIdx >= preKVNum) {
                    ...
                    epilogueRescaleO(gO[gmOffsetO], gOTmp[gmOffsetOTmp], gOUpdate[gmOffsetUpdate], gLse[gmOffsetLse],
                                     ...
                                     qNBlockSize, (stackSeqCount - PRE_LAUNCH == 0),
                                     nowkvSIdx + blockStackNum >= kvSLoopNumTotal, curStackTileMod);
#endif
                }
                stackSeqCount++;
            }
