...
inline int64_t CalcTileEdges(uint64_t ubSize, int64_t rowMapElems, int32_t useGather, int64_t T, int64_t totalRows)
{
    ...
    int64_t avail = static_cast<int64_t>(ubSize) - rowMapBytes - tokenBytes - countsBytes - UB_RESERVE_BYTES;
    if (avail <= 0) {
        return 256;
    }
...
inline ge::graphStatus FillTiling(gert::TilingContext *context, Stage stage)
{
    ...
    OP_CHECK_IF(GetWorkspaceSize(context, T, B, maxKv) != ge::GRAPH_SUCCESS, OP_LOGE(context, "GetWorkspaceSize error"),
                return ge::GRAPH_FAILED);
