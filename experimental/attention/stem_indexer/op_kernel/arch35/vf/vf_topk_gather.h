...
/**
 * @brief SITopKVF 对一个validLen的输入进行topk算法，输出idx_tmp
 ...
 * @param topK topK元素
 * @param validLen 有效元素个数:SICommon::Align(topkCountAlign256_ + validTrunkLen, (uint32_t)256)
 */
template<bool ISOUTVALUE>
__aicore__ inline void SiTopKVF(const LocalTensor<uint32_t>& tmpIdxLocal,
                                ...
                                uint32_t validLen)
{
    ...
    // filter
    AscendC::Duplicate(tmpIdxLocal, (uint32_t)(0), SICommon::Align(topK, (uint32_t)64));
    // 输出大于k-value的值idx
