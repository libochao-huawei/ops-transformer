 ...
 * @param relHOptional        [IN]  可选。分解式相对位置偏置高度轴tensor。数据类型与q一致。
 *                                  shape=(B, N_q, S1, h)，h为key序列网格高度。
 * @param relWOptional        [IN]  可选。分解式相对位置偏置宽度轴tensor。数据类型与q一致。
 *                                  shape=(B, N_q, S1, w)，w为key序列网格宽度，S2=h*w。
