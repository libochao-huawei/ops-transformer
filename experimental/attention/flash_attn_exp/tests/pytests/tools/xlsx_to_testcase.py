...
def convert_row(headers, row_values):
    ...
    for col_name, val in raw.items():
        ...
        elif key in ("q_range", "k_range", "v_range", "rel_range"):
        ...
        # ---- 字符串透传 (rel_mode, atten_mask_dtype, atten_mask_layout 等) ----
        elif key in ("rel_mode", "atten_mask_dtype", "atten_mask_layout"):
            case_dict[key] = [val_str]
...
def xlsx_to_testcase(input_xlsx, output_py, sheet_name=None):
    ...
    if not has_key_col:
        print("[ERROR] 缺少必需列: Testcase_Name 或 testcase_name")
        wb.close()
        sys.exit(1)
    ...
    with open(output_py, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"# Auto-generated from {os.path.basename(input_xlsx)} by xlsx_to_testcase.py\n"
        )
...
""",
    ...
    p.add_argument(
        "--sheet", "-s", default=None, help="xlsx sheet 名称 (默认: 活动 sheet)"
    )
    args = p.parse_args()
    ...
    if output is None:
        ...
        output = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), f"{safe_base}.py"
        )
