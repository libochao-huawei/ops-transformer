    ...
    def _resolve(self, item, params, layout):
        ...
        for token in shape_str.split(","):
            ...
            if token in params:
                ...
                dims.append(params.get("H", 1))
            elif token == "W":
                dims.append(params.get("W", 1))
            else:
...
flash_attn_inputs = (
    ...
    # rel_h / rel_w — 分解式相对位置偏置，按 layout_q 生成
    # rel_mode="zero" 生成全 0 偏置（零偏置等价性用例）
    # rel_range=(lo, hi) 覆盖默认 [-1, 1] 幅值（数值稳定性用例）
    .tensor(
        "rel_h",
        layout={"BNSD": "B,N1,S1,H", "BSND": "B,S1,N1,H", "TND": "total_s1,N1,H"},
        method="zeros",
        when=lambda p: p.get("H") is not None and p.get("rel_mode") == "zero",
    )
    .tensor(
        "rel_w",
        layout={"BNSD": "B,N1,S1,W", "BSND": "B,S1,N1,W", "TND": "total_s1,N1,W"},
        method="zeros",
        when=lambda p: p.get("W") is not None and p.get("rel_mode") == "zero",
    )
    .tensor(
        "rel_h",
        layout={"BNSD": "B,N1,S1,H", "BSND": "B,S1,N1,H", "TND": "total_s1,N1,H"},
