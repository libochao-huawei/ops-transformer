...
def _attend(
    q, k, v, atten_mask, scale, need_fix_invalid, rel_h_bias=None, rel_w_bias=None
):
    q = q.float()
    k = k.float()
    v = v.float()
    ...
    if rel_h_bias is not None and rel_w_bias is not None:
        h_size = rel_h_bias.shape[-1]
        w_size = rel_w_bias.shape[-1]
        rel_h_exp = rel_h_bias.unsqueeze(-1).expand(
            *rel_h_bias.shape[:-1], h_size, w_size
        )
        rel_h_exp = rel_h_exp.reshape(*rel_h_exp.shape[:-2], h_size * w_size)
        rel_w_exp = rel_w_bias.unsqueeze(-2).expand(
            *rel_w_bias.shape[:-1], h_size, w_size
        )
        rel_w_exp = rel_w_exp.reshape(*rel_w_exp.shape[:-2], h_size * w_size)
        bias = rel_h_exp + rel_w_exp
        qk = qk + bias[..., : qk.shape[-1]]
...
def tforward_tnd(q, k, v, **kwargs):
    ...
    rel_h = kwargs.get("rel_h", None)
    rel_w = kwargs.get("rel_w", None)
    ...
    for i in range(b):
        ...
        rel_h_i = rel_w_i = None
        if rel_h is not None and rel_w is not None:
            rel_h_i = rel_h[:, :, q_start : q_start + act_q_len, :]
            rel_w_i = rel_w[:, :, q_start : q_start + act_q_len, :]
        ...
        outi, x_maxi, x_sumi = _attend(
            qi, ki, vi, atten_maski, scale, need_fix, rel_h_i, rel_w_i
        )
...
def tforward(q, k, v, **kwargs):
    ...
    if input_layout != "TND":
        ...
        if rel_h is not None:
            h_dim = rel_h.shape[-1]
            rel_h = (
                rel_h.reshape(1, b, n1, s1, h_dim)
                .transpose(1, 2)
                .reshape(1, n1, b * s1, h_dim)
            )
            kwargs["rel_h"] = rel_h
        if rel_w is not None:
            w_dim = rel_w.shape[-1]
            rel_w = (
                rel_w.reshape(1, b, n1, s1, w_dim)
                .transpose(1, 2)
                .reshape(1, n1, b * s1, w_dim)
            )
            kwargs["rel_w"] = rel_w
