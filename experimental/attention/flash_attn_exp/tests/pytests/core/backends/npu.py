...
try:
    from cann_ops_transformer.ops import flash_attn_exp, flash_attn_exp_metadata
...
class FlashAttnGraphNetwork(torch.nn.Module):
    ...
    def forward(
        ...
        out, lse_out = torch.ops.cann_ops_transformer.flash_attn_exp(
            q,
            k,
            v,
            rel_h,
...
_FD_FIELDS = ["BN2_IDX", "M_IDX", "WS_IDX", "WS_NUM", "M_START", "M_NUM"]
...
def print_metadata(metadata):
    ...
    for sec in range(section_num):
        print(f"\n{DASH}")
        print(
            f"  [Section {sec}] FA Metadata — AIC cores  "
        ...
        print(
            f"  {'':<7}"
            + "".join(f"{'[' + str(i) + ']':>{cw}}" for i in range(len(_FA_FIELDS)))
        )
    ...
    for sec in range(section_num):
        print(f"\n{DASH}")
        print(
            f"  [Section {sec}] FD Metadata — active AIV cores only  "
        ...
        print(
            f"  {'':<7}"
            + "".join(f"{'[' + str(i) + ']':>{cw}}" for i in range(len(_FD_FIELDS)))
        )
...
class NPUBackend(Backend):
    ...
    def __init__(
        self, device_id: int = 0, meta_only: bool = False, graph_mode: bool = False
    ):
    ...
    def compute(
        ...
        metadata = flash_attn_exp_metadata(**meta_kwargs)
        ...
        if self._meta_only:
            ...
            dummy_out = torch.zeros(
                bs, n1, s1, d, dtype=inputs["q"].dtype, device="cpu"
            )
        ...
        if self._graph_mode:
            ...
            rel_h_t = inputs.get("rel_h")
            rel_w_t = inputs.get("rel_w")
            if rel_h_t is not None:
                rel_h_t = rel_h_t.to(self._device)
            if rel_w_t is not None:
                rel_w_t = rel_w_t.to(self._device)
            out, lse = flash_attn_exp(
                inputs["q"],
                inputs["k"],
                inputs["v"],
                rel_h_t,
