...
void FaInfoParser::GetOptionalInputParaRelHwInfo()
{
    opParamInfo_.relH.tensor = context_->GetOptionalInputTensor(REL_H_INDEX);
    opParamInfo_.relH.desc = context_->GetOptionalInputDesc(REL_H_INDEX);
    opParamInfo_.relW.tensor = context_->GetOptionalInputTensor(REL_W_INDEX);
    opParamInfo_.relW.desc = context_->GetOptionalInputDesc(REL_W_INDEX);
}
...
ge::graphStatus FaInfoParser::GetRelHwInfo()
{
    if (opParamInfo_.relH.tensor == nullptr || opParamInfo_.relW.tensor == nullptr) {
        relHSize_ = 0;
        relWSize_ = 0;
        relHwFlag_ = false;
        return ge::GRAPH_SUCCESS;
    }
    const auto &relHShape = opParamInfo_.relH.tensor->GetStorageShape();
    const auto &relWShape = opParamInfo_.relW.tensor->GetStorageShape();
    if (relHShape.GetDimNum() == 0 || relWShape.GetDimNum() == 0) {
        relHSize_ = 0;
        relWSize_ = 0;
        relHwFlag_ = false;
        return ge::GRAPH_SUCCESS;
    }
    relHSize_ = relHShape.GetDim(relHShape.GetDimNum() - 1);
    relWSize_ = relWShape.GetDim(relWShape.GetDimNum() - 1);
    relHwFlag_ = true;
    return ge::GRAPH_SUCCESS;
}
...
void FaInfoParser::GenerateAxisInfo(FaTilingInfo &faInfo)
{
    ...
    faInfo.relHSize = relHSize_;
    faInfo.relWSize = relWSize_;
    faInfo.relHwFlag = relHwFlag_;
}
...
ge::graphStatus FaInfoParser::ParseAxisInfo()
{
    ...
    if (ge::GRAPH_SUCCESS != GetRelHwInfo()) {
        return ge::GRAPH_FAILED;
    }
