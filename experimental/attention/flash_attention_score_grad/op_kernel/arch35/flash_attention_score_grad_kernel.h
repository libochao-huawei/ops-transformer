    ...
    if constexpr (BaseClass::IS_DK_WRITE_UB) {
        ...
        if (!prevRunInfo.isNextS2IdxNoChange) {
            if ASCEND_IS_AIC {
                ...
                CrossCoreWaitFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_C4_TO_V6_FLAG);
            }
    ...
    if constexpr (BaseClass::IS_DV_WRITE_UB) {
        ...
        if (!prevRunInfo.isNextS2IdxNoChange) {
            if ASCEND_IS_AIC {
                ...
                CrossCoreWaitFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_C3_TO_V5_FLAG);
            }
            ...
            if ASCEND_IS_AIV {
                CrossCoreSetFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_MM_RES_REUSE_FLAG);
            }
...
__aicore__ inline void FlashAttentionScoreGradKernel<CubeBlockType, VecBlockType>::ProcessPreloadTwoTimes()
{
    ...
    while (true) {
        ...
        if (!isLastTwoLoop) {
            ...
            if ASCEND_IS_AIC {
                if (taskId >= 1) {
                    CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(EventId::SYNC_V2_TO_C2_FLAG[taskId & 1]);
                    CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(16 + EventId::SYNC_V2_TO_C2_FLAG[taskId & 1]);
                }
            }
            LocalTensor<CALC_TYPE> mm2ResTensor =
            ...
            if ASCEND_IS_AIC {
                if (taskId >= 1) {
                    CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(EventId::SYNC_V2_TO_C1_FLAG[taskId & 1]);
                    CrossCoreWaitFlag<SYNC_MODE, PIPE_FIX>(16 + EventId::SYNC_V2_TO_C1_FLAG[taskId & 1]);
                }
            }
            LocalTensor<CALC_TYPE> mm1ResTensor =
        ...
        if (taskId > 0 && lastTwoLoopCount <= 1) {
            if ASCEND_IS_AIV {
                FagRunInfo &vecRunInfo = runInfos[(taskId + NUM_TWO) % PRELOAD_TIMES];
                LocalTensor<CALC_TYPE> mm1ResTensor =
                    this->mm1ResBuf[vecRunInfo.commonRunInfo.taskIdMod2].template Get<CALC_TYPE>();
                LocalTensor<CALC_TYPE> mm2ResTensor =
                    this->mm2ResBuf[vecRunInfo.commonRunInfo.taskIdMod2].template Get<CALC_TYPE>();
                // wait mm2 result
                CrossCoreWaitFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_C2_TO_V2_FLAG[(taskId + 1) & 1]);
                this->vecBlock.ProcessVec2(mm2ResTensor, this->constInfo,
                                           vecRunInfo); // v2: pse + attenMask + simpleSoftmax
                // wait mm1 result
                CrossCoreWaitFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_C1_TO_V2_FLAG[(taskId + 1) & 1]);
                if (unlikely(this->constInfo.isSink && !IS_DROP)) {
                    this->vecBlock.ProcessVecSink(mm1ResTensor, mm2ResTensor, this->constInfo, vecRunInfo);
                }

                // ------------------------ dv dependency --------------------------
                const uint8_t l1BufferIdx = vecRunInfo.commonRunInfo.taskIdMod2;
                // The first two vector tasks fill different L1 buffers. Wait only when the same buffer is reused.
                if (vecRunInfo.commonRunInfo.taskId >= NUM_TWO) {
                    CrossCoreWaitFlag<SYNC_MODE, PIPE_MTE3>(
                        EventId::SYNC_UB2L1_P_FLAG[l1BufferIdx]); // p must wait dv mte1 copy over
                }
                MutexBuffer<BufferType::L1, SyncType::NO_SYNC> dSL1Buffer = this->dSL1Buf.Get();
                MutexBuffer<BufferType::L1, SyncType::NO_SYNC> pL1Buffer = this->pL1Buf.Get();
                this->vecBlock.ProcessVec4(pL1Buffer, mm2ResTensor, this->constInfo,
                                           vecRunInfo); // v4: cast + nd2nz
                CrossCoreSetFlag<SYNC_MODE, PIPE_MTE3>(
                    EventId::SYNC_UB2L1_P_FLAG[l1BufferIdx]); // dv must wait p copy completely

                // ------------------------ dqk dependency --------------------------
                if (vecRunInfo.commonRunInfo.taskId >= NUM_TWO) {
                    CrossCoreWaitFlag<SYNC_MODE, PIPE_MTE3>(
                        EventId::SYNC_UB2L1_DS_FLAG[l1BufferIdx]); // ds must wait dqk mte1 copy over
                }
                this->vecBlock.ProcessVec3(dSL1Buffer, mm1ResTensor, mm2ResTensor, this->constInfo,
                                           vecRunInfo); // v3: dropout + cast + nd2nz
                if (unlikely(this->constInfo.isSink && IS_DROP)) {
                    this->vecBlock.ProcessVecSink(mm1ResTensor, mm2ResTensor, this->constInfo, vecRunInfo);
                }
                CrossCoreSetFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_V2_TO_C1_FLAG[taskId & 1]);
                CrossCoreSetFlag<SYNC_MODE, PIPE_V>(EventId::SYNC_V2_TO_C2_FLAG[taskId & 1]);
                CrossCoreSetFlag<SYNC_MODE, PIPE_MTE3>(
                    EventId::SYNC_UB2L1_DS_FLAG[l1BufferIdx]); // dqk must wait ds copy completely
            }
        }
        if (taskId > 1) {
            if ASCEND_IS_AIC {
                ...
                LocalTensor<CALC_TYPE> mm2ResTensor =
                    this->mm2ResBuf[prevRunInfo.commonRunInfo.taskIdMod2].template Get<CALC_TYPE>();
                const uint8_t l1BufferIdx = prevRunInfo.commonRunInfo.taskIdMod2;

                // wait p in ub copy to l1
                CrossCoreWaitFlag<SYNC_MODE, PIPE_MTE1>(EventId::SYNC_UB2L1_P_FLAG[l1BufferIdx]);
                CrossCoreWaitFlag<SYNC_MODE, PIPE_MTE1>(16 + EventId::SYNC_UB2L1_P_FLAG[l1BufferIdx]);
                ...
                CrossCoreSetFlag<SYNC_MODE, PIPE_MTE1>(
                    EventId::SYNC_UB2L1_P_FLAG[l1BufferIdx]); // v4 must wait dv mte1 copy over
                CrossCoreSetFlag<SYNC_MODE, PIPE_MTE1>(16 + EventId::SYNC_UB2L1_P_FLAG[l1BufferIdx]);
                // wait ds in ub copy to l1
                CrossCoreWaitFlag<SYNC_MODE, PIPE_MTE1>(EventId::SYNC_UB2L1_DS_FLAG[l1BufferIdx]);
                CrossCoreWaitFlag<SYNC_MODE, PIPE_MTE1>(16 + EventId::SYNC_UB2L1_DS_FLAG[l1BufferIdx]);
                // compute dq
                ...
                this->cubeBlock.template IterateMmDsQ<CALC_TYPE, BaseClass::IS_DK_WRITE_UB>(this->dkWorkSpaceGm,
                                                                                            ...
                                                                                            prevRunInfo); // c4
                CrossCoreSetFlag<SYNC_MODE, PIPE_MTE1>(
                    EventId::SYNC_UB2L1_DS_FLAG[l1BufferIdx]); // v3 must wait dqk mte1 copy over
                CrossCoreSetFlag<SYNC_MODE, PIPE_MTE1>(16 + EventId::SYNC_UB2L1_DS_FLAG[l1BufferIdx]);
            }
        }
        if (isLastTwoLoop) {
            lastTwoLoopCount++;
