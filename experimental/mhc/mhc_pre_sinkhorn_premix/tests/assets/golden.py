...
"""NumPy/Torch golden implementation for MhcPreSinkhornPremix ST cases."""
...
__golden__ = {
    "aclnn": {"aclnnMhcPreSinkhornPremix": "aclnn_mhc_pre_sinkhorn_premix_golden"},
    "kernel": {"mhc_pre_sinkhorn_premix": "mhc_pre_sinkhorn_premix_golden"},
}

__input__ = {
    "aclnn": {"aclnnMhcPreSinkhornPremix": "aclnn_mhc_pre_sinkhorn_premix_input"},
    "kernel": {"mhc_pre_sinkhorn_premix": "mhc_pre_sinkhorn_premix_input"},
}
    ...
    if use_torch and hasattr(template, "device"):
        ...
        try:
            ...
            result = torch.as_tensor(
                np.asarray(value, dtype=np.float32), device=template.device
            )
...
def _fill_tensor(tensor, value):
    ...
    if isinstance(tensor, np.ndarray):
        tensor[...] = np.asarray(value, dtype=np.float32).astype(
...
def _compute(
    ...
    h_pre = (
        _sigmoid(normalized[..., :hc_mult] * alpha_f32[0] + bias_f32[:hc_mult]) + hc_eps
    )
    h_post = 2.0 * _sigmoid(
    ...
    residual_logits = (
        normalized[..., 2 * hc_mult :] * alpha_f32[2] + bias_f32[2 * hc_mult :]
    ).reshape(*batch_shape, hc_mult, hc_mult)

    sum_out = np.empty((2 * num_iters, *batch_shape, hc_mult), dtype=np.float32)
    norm_out = np.empty(
        (2 * num_iters, *batch_shape, hc_mult, hc_mult), dtype=np.float32
    )
...
def mhc_pre_sinkhorn_premix_golden(
    x,
    phi,
    alpha,
    bias,
    ...
    **kwargs,
    ...
    del kwargs
    return _compute(
        x, phi, alpha, bias, hc_mult, num_iters, hc_eps, norm_eps, need_backward
    )


def mhc_pre_sinkhorn_premix_input(x, phi, alpha, bias, hc_mult=4, **kwargs):
    del kwargs
    _set_stable_inputs(x, phi, alpha, bias, int(hc_mult))
    return x, phi, alpha, bias


def aclnn_mhc_pre_sinkhorn_premix_golden(
    x,
    phi,
    alpha,
    bias,
    ...
    **kwargs,
    ...
    return tuple(
        _to_output(value, template, use_torch)
        for value, template in zip(result, templates)
    )


def aclnn_mhc_pre_sinkhorn_premix_input(
    x,
    phi,
    alpha,
    bias,
