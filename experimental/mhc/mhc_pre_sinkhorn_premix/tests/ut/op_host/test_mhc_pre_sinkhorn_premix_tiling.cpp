...
#include "../../../op_host/op_tiling/mhc_pre_sinkhorn_premix_tiling.h"
                                                                   ...
                                                                   void *compileInfo)
{
    ...
    if (baseline.hasPremix) {
        inputs.push_back({{{b, s, HC_MULT}, {b, s, HC_MULT}}, ge::DT_FLOAT, ge::FORMAT_ND});
    }
    std::unique_ptr<gert::TilingContextPara> context(new gert::TilingContextPara(
        "MhcPreSinkhornPremix", inputs,
        {
...
void DeserializeRegbaseTilingData(const TilingInfo &tilingInfo,
                                  optiling::MhcPreSinkhornPremixRegbaseTilingData &tilingData)
{
    size_t offset = 0;
#define READ_TILING_FIELD(type, field) \
    ...
    READ_TILING_FIELD(int64_t, hasPremix);
...
void ExpectRegbaseTilingData(const RegbaseTilingBaseline &baseline)
{
    ...
    optiling::MhcPreSinkhornPremixCompileInfo compileInfo = {baseline.coreNum, baseline.ubSize, SYS_WORKSPACE_SIZE};
    ...
    optiling::MhcPreSinkhornPremixRegbaseTilingData actual;
...
#define EXPECT_TILING_FIELD(field, expected) EXPECT_EQ(actual.get_##field(), (expected)) << baseline.name << ": " #field
    ...
    EXPECT_TILING_FIELD(hasPremix, baseline.hasPremix ? 1 : 0);
#undef EXPECT_TILING_FIELD
}

// premix 用例的不变量校验：tiling 成功、tilingKey 正确、hasPremix 字段为 1
void ExpectRegbasePremixTiling(const char *name, int64_t sequenceLength, int64_t d, bool needBackward,
                               uint64_t expectedTilingKey)
{
    RegbaseTilingBaseline baseline{name, sequenceLength, d, needBackward, 262144, 0, 0, {}};
    baseline.hasPremix = true;

    TilingInfo tilingInfo;
    optiling::MhcPreSinkhornPremixCompileInfo compileInfo = {baseline.coreNum, baseline.ubSize, SYS_WORKSPACE_SIZE};
    auto tilingContext = BuildRegbaseTilingContext(baseline, &compileInfo);
    ASSERT_TRUE(ExecuteTiling(*tilingContext, tilingInfo)) << name;
    EXPECT_EQ(tilingInfo.tilingKey, expectedTilingKey) << name;
    optiling::MhcPreSinkhornPremixRegbaseTilingData actual;
    ASSERT_EQ(tilingInfo.tilingDataSize, actual.GetDataSize()) << name;
    DeserializeRegbaseTilingData(tilingInfo, actual);
    EXPECT_EQ(actual.get_hasPremix(), 1) << name;
}
...
class MhcPreSinkhornPremixTiling : public testing::Test {
protected:
    static void SetUpTestCase()
    {
        std::cout << "MhcPreSinkhornPremixTiling SetUp" << std::endl;
    }

    static void TearDownTestCase()
    {
        std::cout << "MhcPreSinkhornPremixTiling TearDown" << std::endl;
    }
};

TEST_F(MhcPreSinkhornPremixTiling, ascend950_regbase_tilingdata_matches_pre_refactor)
{
...
TEST_F(MhcPreSinkhornPremixTiling, ascend950_regbase_premix_optional_input)
{
    // premix 可选输入：传入时 hasPremix=1，tiling 成功；K/M 分核、grad/no-grad 各覆盖
    ExpectRegbasePremixTiling("k_split_premix_no_grad", 256, 4096, false, 1000);
    ExpectRegbasePremixTiling("k_split_premix_grad", 256, 7168, true, 2000);
    ExpectRegbasePremixTiling("m_split_premix_no_grad", 16384, 4096, false, 1001);
...
TEST_F(MhcPreSinkhornPremixTiling, membase_rejects_premix)
{
    ...
    optiling::MhcPreSinkhornPremixCompileInfo compileInfo = {};

    gert::TilingContextPara tilingContextPara(
        "MhcPreSinkhornPremix",
        {
            {{{B, S, N, C}, {B, S, N, C}}, ge::DT_BF16, ge::FORMAT_ND},
            {{{hcMix, N * C}, {hcMix, N * C}}, ge::DT_FLOAT, ge::FORMAT_ND},
            {{{3}, {3}}, ge::DT_FLOAT, ge::FORMAT_ND},
        ...
        },
        {
            ...
            {{{B, S, 1}, {B, S, 1}}, ge::DT_FLOAT, ge::FORMAT_ND},
            {{{NUM_ITERS * 2, B, S, N}, {NUM_ITERS * 2, B, S, N}}, ge::DT_FLOAT, ge::FORMAT_ND},
            {{{NUM_ITERS * 2, B, S, N, N}, {NUM_ITERS * 2, B, S, N, N}}, ge::DT_FLOAT, ge::FORMAT_ND},
        },
        {
...
TEST_F(MhcPreSinkhornPremixTiling, ascend950_regbase_core_tiling_boundary_branches)
{
...
TEST_F(MhcPreSinkhornPremixTiling, test_tiling_B1_S1024_N4_C512)
{
    int64_t B = 1;
    int64_t S = 1024;
    int64_t N = 4;
...
TEST_F(MhcPreSinkhornPremixTiling, test_tiling_3d_S1024_N4_C512)
{
    int64_t T = 1024;
    int64_t N = 4;
    int64_t C = 512;
...
TEST_F(MhcPreSinkhornPremixTiling, test_tiling_B2_S128_N4_C256)
{
    int64_t B = 2;
    int64_t S = 128;
    int64_t N = 4;
...
TEST_F(MhcPreSinkhornPremixTiling, test_tiling_no_backward_B1_S256_N4_C512)
{
    int64_t B = 1;
    int64_t S = 256;
    int64_t N = 4;
