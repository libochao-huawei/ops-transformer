...
/*!
 * \file mhc_pre_sinkhorn_premix_m_split_core_gradout_arch35.h
 * \brief
 */

#ifndef MHC_PRE_SINKHORN_PREMIX_M_SPLIT_CORE_GRADOUT_ARCH35_H
#define MHC_PRE_SINKHORN_PREMIX_M_SPLIT_CORE_GRADOUT_ARCH35_H
...
template <typename T>
class MhcPreSinkhornPremixMSplitCorePart1Gradout {
public:
    __aicore__ inline MhcPreSinkhornPremixMSplitCorePart1Gradout() {}
        ...
        for (uint64_t roundIdx = 0; roundIdx < singleCoreRound; mGmOffset += tilingData->mL1Size, ++roundIdx) {
            ...
            if ASCEND_IS_AIV {
                ...
                if (GetBlockIdx() % 2 != 0) {
                    ...
                    preMixSplitOffset = CeilDiv(mL1RealSize, 2) * tilingData->hcMult;
                    // 反向输出的split offset
            ...
            if ASCEND_IS_AIV {
                ...
                if (hasPremix_) {
                    tbufPool1.InitBuffer(
                        preMixQue, 1, tilingData->rowInnerFactor * RoundUp<float>(tilingData->hcMix) * sizeof(float));
                }
                ...
                for (int64_t innerRowIdx = 0; innerRowIdx < currentRow; innerRowIdx += tilingData->rowInnerFactor) {
                    ...
                    if (hasPremix_) {
                        ...
                        CopyInWithUbStride(preMixGm[(mGmOffset * tilingData->hcMult) + preMixSplitOffset +
                                                    innerRowIdx * tilingData->hcMult],
                                           preMixLocal, currentInnerRowFactor, tilingData->hcMult, 0,
                    ...
                    for (int64_t dLoopIdx = 0; dLoopIdx < tilingData->dLoop; dLoopIdx++) {
                        ...
                        VFProcessY(yLocal, yMixLocal, xLocal, currentInnerRowFactor, tilingData->hcMult, curDFactor,
                                   tilingData->hcMix);
                        xQue.template FreeTensor(xLocal);
