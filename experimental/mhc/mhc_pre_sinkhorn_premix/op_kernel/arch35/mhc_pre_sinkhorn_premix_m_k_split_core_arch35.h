...
/*!
 * \file mhc_pre_sinkhorn_premix_m_k_split_core_arch35.h
 * \brief
 */

#ifndef MHC_PRE_SINKHORN_PREMIX_M_K_SPLIT_CORE_ARCH35_H
#define MHC_PRE_SINKHORN_PREMIX_M_K_SPLIT_CORE_ARCH35_H
...
class MhcPreSinkhornPremixMPMDSinkhorn {
public:
    __aicore__ inline void Init(GM_ADDR scale, GM_ADDR bias, GM_ADDR output, GM_ADDR workspace,
                                const MhcPreSinkhornPremixRegbaseTilingData *data, TPipe *pipe)
    {
        td = data;
...
private:
    ...
    const MhcPreSinkhornPremixRegbaseTilingData *td;
    int64_t mmLocalSize;
...
template <typename T>
class MhcPreSinkhornPremixMKSplitCorePart1 {
public:
    __aicore__ inline MhcPreSinkhornPremixMKSplitCorePart1() {}
...
template <typename T>
class MhcPreSinkhornPremixMKSplitCorePart2 {
public:
    __aicore__ inline MhcPreSinkhornPremixMKSplitCorePart2() {}

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR hcScale, GM_ADDR hcBase, GM_ADDR premix, GM_ADDR y, GM_ADDR post,
                                GM_ADDR combFrag, GM_ADDR workspace,
                                const MhcPreSinkhornPremixRegbaseTilingData *tilingDataPtr, TPipe *pipePtr)
    {
        pipe = pipePtr;
        tilingData = tilingDataPtr;
        ...
        if ASCEND_IS_AIV {
            ...
            for (int64_t rowOuterIdx = 0; rowOuterIdx < rowOuterLoop; rowOuterIdx++) {
                ...
                VFProcessInvRmsPart3WithGroupReduce(mixesLocal, rmsAndmmLocal, rmsAndmmLocal[mmLocalSize],
                                                    tilingData->normEps, tilingData->kBlockFactor, curRowFactor,
                                                    tilingData->hcMix);
...
private:
    MhcPreSinkhornPremixMPMDSinkhorn sinkhorn;
    TPipe *pipe;
    const MhcPreSinkhornPremixRegbaseTilingData *tilingData;
