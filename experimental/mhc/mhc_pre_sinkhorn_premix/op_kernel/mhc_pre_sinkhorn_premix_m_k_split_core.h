...
/*!
 * \file mhc_pre_sinkhorn_premix_m_split_core.h
 * \brief
 */

#ifndef MHC_PRE_SINKHORN_PREMIX_M_K_SPLIT_A3_CORE_H
#define MHC_PRE_SINKHORN_PREMIX_M_K_SPLIT_A3_CORE_H

#include "kernel_operator.h"
#include "mhc_pre_sinkhorn_premix_base.h"
#include "mhc_pre_sinkhorn_premix_cube_compute.h"

namespace MhcPreSinkhornPremix {
using namespace AscendC;
template <typename T>
class MhcPreSinkhornPremixMembaseKSplitCorePart1 {
public:
    __aicore__ inline MhcPreSinkhornPremixMembaseKSplitCorePart1() {}

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR hcFn, GM_ADDR workspace,
                                const MhcPreSinkhornPremixTilingData *tilingDataPtr, TPipe *pipePtr, int64_t bsOffset,
                                int64_t curBs, bool isTailBsLoop = false)
    {
        pipe = pipePtr;
        ...
        for (uint64_t mGmOffset = mBlkDimIdx * curML1Size; mGmOffset < curBs;
            ...
            for (uint64_t kGmBaseOffset = kGmStartOffset; kGmBaseOffset < kGmEndOffset;
                ...
                if ASCEND_IS_AIC {
                    ...
                    cubeCompute_.ComputeDecode(
                        xCastFp32WsGm[cvLoopIdx_ % DOUBLE_BUFFER * xCastFp32BufSize_],
                        squareSumFp32WsGm[kBlkDimIdx * squareSumFp32BufSize_ + mGmOffset * BLOCK_CUBE],
                        mmOutFp32WsGm[kBlkDimIdx * mmOutFp32BufSize_ + mGmOffset * mmOuterInnerSize_], mmParams);
                    ...
                    for (int64_t i = 0; i < curUbLoops; ++i) {
                        ...
                        int64_t curGlobalxOffset =
                            (mVectorOffset + i * tilingData->stage1MFactor) * tilingData->k + kGmBaseOffset;
                        ...
                        int64_t cutMmInOffset =
                            cvLoopIdx_ % DOUBLE_BUFFER * xCastFp32BufSize_ +
                            (i * tilingData->stage1MFactor + mVectorOffset) * tilingData->cvLoopKSize;
...
private:
    TPipe *pipe;
    const MhcPreSinkhornPremixTilingData *tilingData;
...
template <typename T>
class MhcPreSinkhornPremixMembaseKSplitCorePart2 {
public:
                                ...
                                int64_t bsOffset, bool isTailBsLoop = false)
    {
        ...
        int64_t mixesQue01Size =
            stage1UsedCoreNum * tilingData->stage2RowFactor * tilingData->hcMultAlign * NUM_TWO * sizeof(float);
        pipe->InitBuffer(mixesQue01, NUM_TWO, mixesQue01Size);
        pipe->InitBuffer(mixesQue2, NUM_TWO,
                         stage1UsedCoreNum * tilingData->stage2RowFactor * tilingData->hcMult *
                             tilingData->hcMultAlign * sizeof(float));
        pipe->InitBuffer(squareSumQue, NUM_TWO,
                         stage1UsedCoreNum * tilingData->stage2RowFactor * SQUARE_SUM_SIZE * sizeof(float));
        ...
        pipe->InitBuffer(hcBrcbBuf1,
                         RoundUp<float>(tilingData->stage2RowFactor * tilingData->hcMultAlign * NUM_TWO) * BLOCK_SIZE);
    ...
    __aicore__ inline void Process(int64_t bsOffset, int64_t curBs, bool isTailBsLoop = false)
    {
        ...
        if ASCEND_IS_AIV {
            ...
            int64_t workspaceSize2 =
                CeilAlign(stage1UsedCoreNum * curBs * mmLastAxisSize * sizeof(float), WORKSPACE_ALIGN_SIZE) /
                sizeof(float);
            ...
            for (int64_t rowOuterIdx = 0; rowOuterIdx < rowOuterLoop; rowOuterIdx++) {
                ...
                CopyIn(workspaceGm[workspaceSize1 + workspaceSize2 +
                                   stage2BlockIdx * curRowOfFormerBlock * SQUARE_SUM_SIZE +
                                   rowOuterIdx * curStage2RowFactor * SQUARE_SUM_SIZE],
                       squareSumOutLocal, stage1UsedCoreNum, curRowFactor * SQUARE_SUM_SIZE,
                       CeilAlign(curBs, SQUARE_SUM_SIZE) * SQUARE_SUM_SIZE - curRowFactor * SQUARE_SUM_SIZE);
                ...
                if (needGrad_) {
                    VToMTE3Sync();
                    CopyOut(rsqrtLocal,
                            invRmsGm[stage2BlockIdx * curRowOfFormerBlock + rowOuterIdx * curStage2RowFactor], 1,
                            curRowFactor);
                }
                ...
                uint64_t mixBaseOffset = workspaceSize1 +
                                         ...
                                         rowOuterIdx * curStage2RowFactor *
                                             CeilAlign(tilingData->hcMix, WORKSPACE_ALIGN_SIZE / sizeof(float));
                CopyInWithOuterFor(workspaceGm[mixBaseOffset], mxies01Local, stage1UsedCoreNum, curRowFactor,
                                   tilingData->hcMult, curBs,
                                   CeilAlign(tilingData->hcMix, WORKSPACE_ALIGN_SIZE / sizeof(float)));
                ...
                ReduceSumARAPerf(mxies01ReduceLocal, mxies01Local, NUM_TWO, stage1UsedCoreNum,
                ...
                for (int64_t dLoopIdx = 0; dLoopIdx < tilingData->dLoop; dLoopIdx++) {
                    ...
                    xLocal = xQue.template AllocTensor<T>();
                    CopyIn(xGm[xGmBlockBaseOffsetPart2 + xGmBsBaseOffsetPart2 + dLoopIdx * tilingData->dFactor], xLocal,
                           curStage2RowFactor * tilingData->hcMult, curDFactor, tilingData->d - curDFactor);
                    xQue.template EnQue(xLocal);
                    xLocal = xQue.template DeQue<T>();
                ...
                for (int64_t i = 0; i < stage1UsedCoreNum; ++i) {
                    for (int64_t j = 0; j < curRowFactor; ++j) {
                        CopyIn(workspaceGm[workspaceSize1 + i * curBs * mmLastAxisSize + j * mmLastAxisSize +
                                           stage2BlockIdx * curRowOfFormerBlock * mmLastAxisSize +
                                           rowOuterIdx * curStage2RowFactor * mmLastAxisSize +
                ...
                ReduceSumARAPerf(mxies02ReduceLocal, mixes2Local, 1, stage1UsedCoreNum,
                ...
                if (needGrad_) {
                    ...
                    for (int64_t i = 0; i < curRowFactor; i++) {
                        CopyOut(mxies02ReduceLocal[i * tilingData->hcMult * tilingData->hcMultAlign],
                                hcBeforeNormGm[hcBeforeNormMix2Offset + tilingData->hcMix * i], tilingData->hcMult,
                                tilingData->hcMult);
                    }
                    MTE3ToVSync();
                }
                ...
                MulABLastDimBrcInline<float, false>(mxies02ReduceLocal, mxies02ReduceLocal, rsqrtLocal, rowBrcbLocal0,
                                                    curRowFactor, tilingData->hcMult * tilingData->hcMultAlign);
                ...
                SoftmaxFP32Perf(mxies02ReduceLocal, mxies02ReduceLocal, reduceLocal, hcBrcbLocal1,
                                curRowFactor * tilingData->hcMult, tilingData->hcMult, tilingData->hcEps);

                if (needGrad_) {
                    VToMTE3Sync();
                    int64_t normOutBaseOffset =
                        (bsOffset + stage2BlockIdx * curRowOfFormerBlock + rowOuterIdx * curStage2RowFactor) *
                        tilingData->hcMult * tilingData->hcMult;
                ...
                if (needGrad_) {
                    VToMTE3Sync();
                    int64_t sumOutBaseOffset = (tilingData->bs + bsOffset + stage2BlockIdx * curRowOfFormerBlock +
                                                rowOuterIdx * curStage2RowFactor) *
                                               tilingData->hcMult;
                    CopyOut(reduceLocal, sumOutGm[sumOutBaseOffset], curRowFactor, tilingData->hcMult);
                    MTE3ToVSync();
                ...
                if (needGrad_) {
                    VToMTE3Sync();
                    int64_t normOutBaseOffset = (tilingData->bs + bsOffset + stage2BlockIdx * curRowOfFormerBlock +
                                                 rowOuterIdx * curStage2RowFactor) *
                                                tilingData->hcMult * tilingData->hcMult;
                ...
                for (int64_t iter = 1; iter < tilingData->iterTimes; iter++) {
                    LastDimReduceSumPerf(reduceLocal, combFragLocal, curRowFactor * tilingData->hcMult,
                                         tilingData->hcMult);
                    Adds(reduceLocal, reduceLocal, tilingData->hcEps, curRowFactor * tilingData->hcMult);
                    ...
                    if (needGrad_) {
                        VToMTE3Sync();
                        int64_t sumOutBaseOffset =
                            ((iter * 2) * tilingData->bs + bsOffset + stage2BlockIdx * curRowOfFormerBlock +
                             rowOuterIdx * curStage2RowFactor) *
                            tilingData->hcMult;
                        CopyOut(reduceLocal, sumOutGm[sumOutBaseOffset], curRowFactor, tilingData->hcMult);
                        MTE3ToVSync();
                    ...
                    if (needGrad_) {
                        int64_t sumOutBaseOffset =
                            ((iter * 2 + 1) * tilingData->bs + bsOffset + stage2BlockIdx * curRowOfFormerBlock +
                             rowOuterIdx * curStage2RowFactor) *
                            tilingData->hcMult;
                        VToMTE3Sync();
                        CopyOut(reduceLocal, sumOutGm[sumOutBaseOffset], curRowFactor, tilingData->hcMult);
