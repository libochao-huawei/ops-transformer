...
if _TORCHAIR_AVAILABLE:
    ...
    def MhcPreSinkhornPremix(
        x: Tensor,
        phi: Tensor,
        alpha: Tensor,
        bias: Tensor,
        premix: Optional[Tensor] = None,
        *,
        hc_mult: int = 4,
        num_iters: int = 20,
        hc_eps: float = 1e-6,
        ...
        .OPTIONAL_INPUT(premix, TensorType({DT_FLOAT}))\n
        ...
        return ge_op(
            op_type="MhcPreSinkhornPremix",
            inputs=inputs,
            attrs=attrs,
            outputs=outputs,
            dependencies=dependencies,
            ir=IrDef("MhcPreSinkhornPremix")
            .input("x", "DT_BF16, DT_FLOAT16")
            .input("phi", "DT_FLOAT")
            .input("alpha", "DT_FLOAT")
            .input("bias", "DT_FLOAT")
            .optional_input("premix", "DT_FLOAT")
    ...
    @register_fx_node_ge_converter(
        torch.ops.cann_ops_transformer.mhc_pre_sinkhorn_premix.default
    )
    def convert_mhc_pre_sinkhorn_premix(
        x: Tensor,
        phi: Tensor,
        alpha: Tensor,
        bias: Tensor,
        premix: Optional[Tensor],
        hc_mult: int,
        num_iters: int,
        hc_eps: float,
        norm_eps: float,
