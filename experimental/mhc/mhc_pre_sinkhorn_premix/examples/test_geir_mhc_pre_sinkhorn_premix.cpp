...
#include "../op_graph/mhc_pre_sinkhorn_premix_proto.h"
...
void SetOutputDesc(ge::op::MhcPreSinkhornPremix &op, DataType xDtype, bool needBackward)
{
    ...
    if (needBackward) {
        ...
        op.update_output_desc_hcBeforeNorm(TensorDesc(ge::Shape({BATCH, SEQ_LEN, HC_MIX}), FORMAT_ND, ge::DT_FLOAT));
        op.update_output_desc_invRms(TensorDesc(ge::Shape({BATCH, SEQ_LEN, 1}), FORMAT_ND, ge::DT_FLOAT));
        op.update_output_desc_sumOut(
            TensorDesc(ge::Shape({NUM_ITERS * 2, BATCH, SEQ_LEN, HC_MULT}), FORMAT_ND, ge::DT_FLOAT));
        op.update_output_desc_normOut(
            TensorDesc(ge::Shape({NUM_ITERS * 2, BATCH, SEQ_LEN, HC_MULT, HC_MULT}), FORMAT_ND, ge::DT_FLOAT));
    } else {
...
{
    auto mhc_pre_sinkhorn_premix = ge::op::MhcPreSinkhornPremix(testCase.name);

    if (AddDataInput("x", 0, testCase.xDtype, {BATCH, SEQ_LEN, HC_MULT, HIDDEN_DIM}, graph, input_tensors, input_ops,
                     mhc_pre_sinkhorn_premix) != SUCCESS) {
        return FAILED;
    }
    ...
    if (AddDataInput("alpha", 2, ge::DT_FLOAT, {3}, graph, input_tensors, input_ops, mhc_pre_sinkhorn_premix) !=
        SUCCESS) {
        return FAILED;
    }
    if (AddDataInput("bias", 3, ge::DT_FLOAT, {HC_MIX}, graph, input_tensors, input_ops, mhc_pre_sinkhorn_premix) !=
        SUCCESS) {
        return FAILED;
    }

    mhc_pre_sinkhorn_premix.set_attr_hc_mult(HC_MULT);
    mhc_pre_sinkhorn_premix.set_attr_num_iters(NUM_ITERS);
    mhc_pre_sinkhorn_premix.set_attr_hc_eps(1e-6f);
    mhc_pre_sinkhorn_premix.set_attr_norm_eps(1e-6f);
    mhc_pre_sinkhorn_premix.set_attr_need_backward(testCase.needBackward);
    SetOutputDesc(mhc_pre_sinkhorn_premix, testCase.xDtype, testCase.needBackward);

    outputs.push_back(mhc_pre_sinkhorn_premix);
    return SUCCESS;
}
...
int RunCase(Session &session, uint32_t graphId, const TestCase &testCase)
{
    ...
    printf("%s - INFO - [XIR]: Run case %s, x dtype: %d, need_backward: %d\n", GetTime().c_str(), testCase.name,
           static_cast<int>(testCase.xDtype), testCase.needBackward);
...
int main()
{
    ...
    std::vector<TestCase> testCases = {
        {ge::DT_BF16, true, "mhc_pre_sinkhorn_premix_bf16_need_backward_true"},
        {ge::DT_BF16, false, "mhc_pre_sinkhorn_premix_bf16_need_backward_false"},
        {ge::DT_FLOAT16, true, "mhc_pre_sinkhorn_premix_fp16_need_backward_true"},
        {ge::DT_FLOAT16, false, "mhc_pre_sinkhorn_premix_fp16_need_backward_false"},
    };
