...
- **算子功能**：本算子为 experimental 版本的 MhcPreSinkhorn，相比正式算子新增 premix 可选输入（仅 Ascend 950PR/950DT 支持）：传入 premix 时，$\mathbf{h}_{\text{in}}$ 使用 premix 作为加权系数（通常填上一轮输出的 $\mathbf{H^{pre}_l}$），而非本轮内部计算的 $\mathbf{H^{pre}_l}$；needBackward 为 true 时，本轮内部计算的 hPre、hcBeforeNorm、invRms、sumOut、normOut 等中间变量仍正常输出，可供反向梯度计算使用。原功能：基于一系列计算得到MHC架构中hidden层的$\mathbf{H}'_{\text{res}}$和$\mathbf{H}_{\text{post}}$投影矩阵以及Attention或MLP层的输入矩阵$\mathbf{h}_{\text{in}}$。对$\mathbf{H}'_{\text{res}}$矩阵执行Sinkhorn迭代归一化变换，最终得到双随机矩阵$\mathbf{H}_{\text{res}}$；支持输出中间计算结果，用于反向梯度计算。包括sigmoid计算之后的$\mathbf{H^{pre}_l}$矩阵、$\vec{x^{'}_{l}}$与$\mathbf{\varphi}$矩阵乘的结果，输入x的RmsNorm结果$\mathbf{\vec{x^{'}_{l}}}$、迭代过程中的中间归一化结果和$\mathbf{normOut}$和求和结果$\mathbf{sumOut}$。
  ...
  <table style="table-layout: auto; width: 100%">
    ...
    <tbody>
      ...
      <tr>
        <td>premix</td>
        <td>可选输入</td>
        <td>premix加权系数（通常填上一轮输出的hPre），shape为x.shape[:-1]。仅Ascend 950PR/950DT支持。</td>
        <td>FLOAT32</td>
        <td>ND</td>
      </tr>
      <tr>
...
- <term>Ascend 950PR/Ascend 950DT</term>：
  ...
  - 支持可选输入premix，shape为(bs, seq_len, n)或(t, n)。
...
| 调用方式      | 调用样例                 | 说明                                                         |
|--------------|-------------------------|--------------------------------------------------------------|
| aclnn调用 | [test_aclnn_mhc_pre_sinkhorn_premix](examples/test_aclnn_mhc_pre_sinkhorn_premix.cpp) | 通过[aclnnMhcPreSinkhornPremix](docs/aclnnMhcPreSinkhornPremix.md)接口方式调用MhcPreSinkhornPremix算子。 |
| PyTorch API | - | 通过[cann_ops_transformer.mhc_pre_sinkhorn_premix](docs/torchapi_mhc_pre_sinkhorn_premix.md)接口方式调用MhcPreSinkhornPremix算子。 |
