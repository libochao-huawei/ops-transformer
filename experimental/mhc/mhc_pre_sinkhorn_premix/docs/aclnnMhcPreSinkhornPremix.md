...
每个算子分为[两段式接口](../../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnMhcPreSinkhornPremixGetWorkspaceSize`接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnMhcPreSinkhornPremix`执行实际计算。

```c++
aclnnStatus aclnnMhcPreSinkhornPremixGetWorkspaceSize(
    const aclTensor *x,
    const aclTensor *phi,
    const aclTensor *alpha,
    const aclTensor *bias,
    const aclTensor *premix,
    int64_t          hcMult,
    int64_t          numIters,
    double           hcEps,
    double           normEps,
...
```c++
aclnnStatus aclnnMhcPreSinkhornPremix(
    void          *workspace,
    uint64_t       workspaceSize,
    aclOpExecutor *executor,
    aclrtStream    stream)
...
## aclnnMhcPreSinkhornPremixGetWorkspaceSize
...
- **参数说明**
  ...
  <table style="undefined;table-layout: fixed; width: 1435px"><colgroup>
    ...
    <tbody>
      ...
      <tr>
        <td>premix（aclTensor*）</td>
        <td>输入</td>
        <td>可选输入， premix加权系数（通常填上一轮输出的hPre）。传入时hin使用premix而非本轮内部计算的hPre进行加权求和；needBackward为true时hPre等中间变量仍正常输出。仅Ascend 950PR/950DT支持。</td>
        <td>可为nullptr（不传）。</td>
        <td>FLOAT32</td>
        <td>ND</td>
        <td>x.shape[:-1]，即(t, n)或(b, s, n)</td>
        <td>√</td>
      </tr>
      <tr>
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnMhcPreSinkhornPremixGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  ...
  - aclnnMhcPreSinkhornPremix默认采用确定性实现，相同输入多次调用结果一致。
...
int main()
{
  ...
  CHECK_RET(aclnn_ret == ACL_SUCCESS, LOG_PRINT("aclnnMhcPreSinkhornPremixGetWorkspaceSize failed, error: %d\n", aclnn_ret);
            return -1);
