...
/*!
 * \file mhc_pre_sinkhorn_premix_tiling_arch35.cpp
 * \brief
 */
...
int64_t CalcUbBufferSize(const UbBufferConfig &config, int64_t rowFactor, int64_t dFactor)
{
    ...
    if (config.hasPremix) {
        totalSize += rowFactor * hcMixAlign * sizeof(float); // premix
    }
...
void PrintFinalTilingData(gert::TilingContext *context, MhcPreSinkhornPremixRegbaseTilingData &tilingData,
                          uint64_t tilingKey, uint64_t blockDim, size_t workspaceSize)
{
    OP_LOGD(context->GetNodeName(),
            "MhcPreSinkhornPremix regbase final tiling summary: tilingKey=%lu, blockDim=%lu, workspaceSize=%zu, "
            "tilingDataSize=%zu",
            tilingKey, blockDim, workspaceSize, tilingData.GetDataSize());
    OP_LOGD(context->GetNodeName(),
            "MhcPreSinkhornPremix regbase final tiling shape: bs=%ld, hcMix=%ld, hcMult=%ld, d=%ld, hcMultAlign=%ld, "
    ...
    OP_LOGD(context->GetNodeName(),
            "MhcPreSinkhornPremix regbase final tiling core: rowOfFormerBlock=%ld, "
    ...
    OP_LOGD(context->GetNodeName(),
            "MhcPreSinkhornPremix regbase final tiling tail: tailRowFactorOfFormerBlock=%ld, "
    ...
    OP_LOGD(context->GetNodeName(),
            "MhcPreSinkhornPremix regbase final tiling split: k=%ld, cubeBlockDimM=%ld, cubeBlockDimK=%ld, "
...
MhcPreSinkhornPremixTilingRegbase::MhcPreSinkhornPremixTilingRegbase(gert::TilingContext *tilingContext)
    : context_(tilingContext)
{}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::GetPlatformInfo()
{
    ...
    OP_LOGD(context_->GetNodeName(),
            "MhcPreSinkhornPremix regbase platform: aivCoreNum=%lu, aicCoreNum=%lu, ubSize=%lu, sysWorkspaceSize=%lu",
            aivCoreNum_, aicCoreNum_, ubSize_, sysWorkspaceSize_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::GetAttr()
{
...
ge::graphStatus MhcPreSinkhornPremixTilingRegbase::GetShapeAttrsInfoInner()
{
    ...
    // premix 为可选输入（index 4），传入时 shape 应为 x.shape[:-1] 展平后的 (bs, hcMult)
    auto shapePremix = context_->GetOptionalInputShape(4);
    hasPremix_ = (shapePremix != nullptr);
    if (hasPremix_) {
        auto &premixStorageShape = shapePremix->GetStorageShape();
        size_t premixDimNum = premixStorageShape.GetDimNum();
        OP_CHECK_IF(
            premixDimNum != xDimNum - 1,
            OP_LOGE(context_->GetNodeName(), "premix dim num should be %zu, but is %zu", xDimNum - 1, premixDimNum),
            return ge::GRAPH_FAILED);
        OP_CHECK_IF(premixStorageShape.GetDim(premixDimNum - 1) != hcMult_,
                    OP_LOGE(context_->GetNodeName(), "premix last dim should be %ld, but is %ld", hcMult_,
                            premixStorageShape.GetDim(premixDimNum - 1)),
                    return ge::GRAPH_FAILED);
    }
    ...
    OP_LOGD(context_->GetNodeName(),
            "MhcPreSinkhornPremix regbase shape attrs: bs=%ld, hcMix=%ld, hcMult=%ld, d=%ld, iterTimes=%ld, "
            "needGrad=%d, hcEps=%g, normEps=%g",
            bs_, hcMix_, hcMult_, d_, iterTimes_, needGrad_ ? 1 : 0, hcEps_, normEps_);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::CalcRegbaseCommonTiling(GradOutMode gradOutMode, SplitMode splitMode)
{
    ...
    bufferConfig.hasPremix = hasPremix_;
    ...
    tilingData_.set_hasPremix(hasPremix_ ? 1 : 0);
    ...
    OP_LOGD(context_->GetNodeName(),
            "MhcPreSinkhornPremix regbase UB tiling: withGradOut=%d, isKSplit=%d, availableUbSize=%ld, "
...
ge::graphStatus MhcPreSinkhornPremixTilingRegbase::CalcRegbaseOpTiling()
{
    return CalcRegbaseCommonTiling(GradOutMode::DISABLED, SplitMode::M_SPLIT);
}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::CalcRegbaseOpGradoutTiling()
{
    return CalcRegbaseCommonTiling(GradOutMode::ENABLED, SplitMode::M_SPLIT);
}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::CalcMKSplitCorePart2Tiling()
{
    return CalcRegbaseCommonTiling(GradOutMode::DISABLED, SplitMode::K_SPLIT);
}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::CalcMKSplitCorePart2GradoutTiling()
{
    return CalcRegbaseCommonTiling(GradOutMode::ENABLED, SplitMode::K_SPLIT);
}

ge::graphStatus MhcPreSinkhornPremixTilingRegbase::CalcOpTiling()
{
    ...
    OP_LOGD(context_->GetNodeName(), "ops: MhcPreSinkhornPremix, deterministic_level=%d", deterministicLevel);
    if (deterministicLevel == BATCH_CONSISTENCY_LEVEL) {
        OP_LOGI(context_->GetNodeName(),
                "MhcPreSinkhornPremix batch consistency enabled: deterministicLevel=%d, force kDimNum from %lu to 1.",
    ...
    OP_LOGD(context_->GetNodeName(),
            "MhcPreSinkhornPremix regbase tiling decision: tilingKey=%lu, needGrad=%d, deterministicLevel=%d, "
...
ge::graphStatus MhcPreSinkhornPremixTilingRegbase::DoOpTiling()
{
...
ge::graphStatus MhcPreSinkhornPremixTilingRegbase::GetWorkspaceSize()
{
...
ge::graphStatus MhcPreSinkhornPremixTilingRegbase::PostTiling()
{
