...
/*!
 * \file mhc_pre_sinkhorn_premix_infershape.cpp
 * \brief MhcPreSinkhornPremix infershape implementation
 */
...
constexpr size_t INDEX_PREMIX = 4;
...
static ge::graphStatus InferShapeForMhcPreSinkhornPremix(gert::InferShapeContext *context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do MhcPreSinkhornPremix infershape.");
    ...
    const gert::Shape *premixShape = context->GetOptionalInputShape(INDEX_PREMIX);
    if (premixShape != nullptr) {
        OP_CHECK_IF(premixShape->GetDimNum() != xDimNum - 1,
                    OP_LOGE(context->GetNodeName(), "premix dim must be x dim - 1 (%zu), but got %zu.", xDimNum - 1,
                            premixShape->GetDimNum()),
                    return ge::GRAPH_FAILED);
        for (size_t i = 0; i + 1 < xDimNum; i++) {
            OP_CHECK_IF(premixShape->GetDim(i) != xShape->GetDim(i),
                        OP_LOGE(context->GetNodeName(), "premix shape[%zu] must equal x shape[%zu] (%ld), but got %ld.",
                                i, i, xShape->GetDim(i), premixShape->GetDim(i)),
                        return ge::GRAPH_FAILED);
        }
    }
    ...
    OP_LOGD(context->GetNodeName(), "End to do MhcPreSinkhornPremix infershape.");
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataTypeForMhcPreSinkhornPremix(gert::InferDataTypeContext *context)
{
    OP_LOGD(context->GetNodeName(), "Begin to do MhcPreSinkhornPremix infer datatype.");
    ...
    OP_LOGD(context->GetNodeName(), "End to do MhcPreSinkhornPremix infer datatype.");
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(MhcPreSinkhornPremix)
    .InferShape(InferShapeForMhcPreSinkhornPremix)
    .InferDataType(InferDataTypeForMhcPreSinkhornPremix);
